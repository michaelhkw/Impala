...
/// reference to the root of a ScalarExpr tree, runtime states (e.g. FunctionContexts)
/// needed during evaluation and also a buffer for the expression evaluation result.
/// A single evaluator is not thread-safe.
///
...
/// the ScalarExpr nodes in the tree. The index of each node's entry is defined in the
/// its 'fn_ctx_idx_' field.
///
/// A ScalarExprEvaluator is created using the Create() interface. It must be initialized
/// by calling Open() before use. Close() must also be called to free up resources owned
/// by the evaluator.
///
class ScalarExprEvaluator {
 public:
  ...
  static Status Create(ObjectPool* pool, RuntimeState* state, MemPool* mem_pool,
      const ScalarExpr* expr, ScalarExprEvaluator** evaluator) WARN_UNUSED_RESULT;
  ...
  static Status Create(ObjectPool* pool, RuntimeState* state, MemPool* mem_pool,
      ...
      std::vector<ScalarExprEvaluator*>* evaluators) WARN_UNUSED_RESULT;
  ...
  static Status Open(std::vector<ScalarExprEvaluator*>& evaluators,
  ...
  static void Close(std::vector<ScalarExprEvaluator*>& evaluators, RuntimeState* state);
  ...
  /// should only be called after Open() has been called on this expr. Returns an error if
  /// there was an error evaluating the expression or if memory could not be allocated for
  ...
  static bool HasLocalAllocations(const std::vector<ScalarExprEvaluator*>& ctxs);
 ...
 private:
  ...
  /// and owned by this ScalarExprEvaluator.
  ...
  /// Call Create() instead.
  ScalarExprEvaluator(const ScalarExpr* root);
