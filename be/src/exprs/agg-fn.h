...
/// --- Implementation
///
/// AggFn contains static states about an aggregate function such as the aggregation
/// operations, pointers to the UDAF interface functions implementing various states of
/// aggregation and the descriptors for the intermediate and output values among other
/// things. Please see udf/udf.h for details of the UDAF interfaces. AggFnEvaluator is
/// the interface for evaluating aggregate functions against input tuple rows. It invokes
/// the following functions at different phases of the aggregation:
///
/// init_fn_     : An initialization function that initializes the aggregate state.
///
...
class AggFn : public Expr {
 public:
  ...
  /// That logic needs to be moved. For now, add some enums for those builtins.
  enum AggregationOp {
    COUNT,
    MIN,
    MAX,
  ...
  /// Creates and initializes an aggregate function from 'texpr' and returns it
  /// in 'agg_fn'. 'row_desc' is the row descriptor of the input tuple row;
  /// 'intermediate_slot_desc' is the slot descriptor of the intermediate value;
  /// 'output_slot_desc' is the slot descriptor of the output value. Returns error
  /// status on failure.
  static Status Create(ObjectPool* pool, RuntimeState* state,
      const RowDescriptor& row_desc, const SlotDescriptor* intermediate_slot_desc,
      const SlotDescriptor* output_slot_desc, const TExpr& texpr, AggFn** agg_fn);
  ...
  virtual void Close();
  static void Close(std::vector<AggFn*>& exprs);
