...
#define IS_NULL_COMPUTE_FUNCTION(type) \
  type IsNullExpr::Get##type( \
    ...
    type val = GetChild(0)->Get##type(evaluator, row);  \
    if (!val.is_null) return val; /* short-circuit */ \
    return GetChild(1)->Get##type(evaluator, row); \
  }
...
#define NULL_IF_COMPUTE_FUNCTION(AnyValType) \
  AnyValType NullIfExpr::Get##AnyValType(ScalarExprEvaluator* evaluator, \
      const TupleRow* row) const { \
    DCHECK_EQ(children_.size(), 2); \
    AnyValType lhs_val = GetChild(0)->Get##AnyValType(evaluator, row); \
    ...
    AnyValType rhs_val = GetChild(1)->Get##AnyValType(evaluator, row); \
    if (!rhs_val.is_null && \
...
#define IF_COMPUTE_FUNCTION(type) \
  type IfExpr::Get##type(ScalarExprEvaluator* evaluator, const TupleRow* row) const { \
    DCHECK_EQ(children_.size(), 3); \
    BooleanVal cond = GetChild(0)->GetBooleanVal(evaluator, row); \
    if (cond.is_null || !cond.val) { \
      return GetChild(2)->Get##type(evaluator, row); \
    } \
    return GetChild(1)->Get##type(evaluator, row); \
  }
...
#define COALESCE_COMPUTE_FUNCTION(type) \
  type CoalesceExpr::Get##type( \
    ...
    for (int i = 0; i < children_.size(); ++i) { \
      type val = GetChild(i)->Get##type(evaluator, row); \
