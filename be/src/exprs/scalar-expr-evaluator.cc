...
ScalarExprEvaluator::ScalarExprEvaluator(const ScalarExpr* root)
  : fn_ctxs_ptr_(nullptr),
    mem_pool_(nullptr),
    root_(root),
    is_clone_(false),
    inited_(false),
...
ScalarExprEvaluator::~ScalarExprEvaluator() {
  DCHECK(!inited_ || closed_);
}

Status ScalarExprEvaluator::Create(ObjectPool* pool, RuntimeState* state,
    MemPool* mem_pool, const ScalarExpr* expr, ScalarExprEvaluator** evaluator) {
  DCHECK(expr != nullptr);
  *evaluator = pool->Add(new ScalarExprEvaluator(expr));
  (*evaluator)->mem_pool_ = mem_pool;
  (*evaluator)->fn_ctxs_.clear();
  if (expr->fn_ctx_idx_end_ > 0) {
    (*evaluator)->fn_ctxs_.resize(expr->fn_ctx_idx_end_, nullptr);
    (*evaluator)->CreateFnCtxs(state, expr);
    for (FunctionContext* fn_ctx : (*evaluator)->fn_ctxs_) DCHECK(fn_ctx != nullptr);
    (*evaluator)->fn_ctxs_ptr_ = (*evaluator)->fn_ctxs_.data();
  } else {
    DCHECK_EQ((*evaluator)->fn_ctxs_.size(), 0);
    DCHECK_EQ(expr->fn_ctx_idx_end_, 0);
    DCHECK_EQ(expr->fn_ctx_idx_, -1);
    (*evaluator)->fn_ctxs_ptr_ = nullptr;
  }
  (*evaluator)->inited_ = true;
  return Status::OK();
}

Status ScalarExprEvaluator::Create(ObjectPool* pool, RuntimeState* state,
    MemPool* mem_pool, const vector<ScalarExpr*>& exprs,
    vector<ScalarExprEvaluator*>* evaluators) {
  for (ScalarExpr* expr : exprs) {
    ScalarExprEvaluator* evaluator;
    Status status = Create(pool, state, mem_pool, expr, &evaluator);
...
Status ScalarExprEvaluator::Open(RuntimeState* state) {
  ...
  return root_->OpenEvaluator(state, this, scope);
}

Status ScalarExprEvaluator::Open(
    vector<ScalarExprEvaluator*>& evaluators, RuntimeState* state) {
  for (int i = 0; i < evaluators.size(); ++i) RETURN_IF_ERROR(evaluators[i]->Open(state));
  return Status::OK();
}
...
void ScalarExprEvaluator::Close(RuntimeState* state) {
  ...
  FunctionContext::FunctionStateScope scope =
      is_clone_ ? FunctionContext::THREAD_LOCAL : FunctionContext::FRAGMENT_LOCAL;
  root_->CloseEvaluator(state, this, scope);
  for (int i = 0; i < fn_ctxs_.size(); ++i) {
    fn_ctxs_[i]->impl()->Close();
    delete fn_ctxs_[i];
  }
  fn_ctxs_.clear();
  // Owner of mem_pool_ needs to call FreeAll() on it.
  closed_ = true;
}
...
Status ScalarExprEvaluator::Clone(ObjectPool* pool, RuntimeState* state,
    MemPool* mem_pool, ScalarExprEvaluator** cloned_evaluator) const {
  DCHECK(inited_);
  DCHECK(opened_);
  *cloned_evaluator = pool->Add(new ScalarExprEvaluator(root_));
  (*cloned_evaluator)->mem_pool_ = mem_pool;
  for (int i = 0; i < fn_ctxs_.size(); ++i) {
    (*cloned_evaluator)->fn_ctxs_.push_back(fn_ctxs_[i]->impl()->Clone(mem_pool));
  }
  (*cloned_evaluator)->fn_ctxs_ptr_ = &((*cloned_evaluator)->fn_ctxs_[0]);
  (*cloned_evaluator)->is_clone_ = true;
  (*cloned_evaluator)->inited_ = true;
  (*cloned_evaluator)->opened_ = true;
  (*cloned_evaluator)->output_scale_ = output_scale_;
  return root_->OpenEvaluator(state, *cloned_evaluator, FunctionContext::THREAD_LOCAL);
}

Status ScalarExprEvaluator::Clone(ObjectPool* pool, RuntimeState* state,
    MemPool* mem_pool, const vector<ScalarExprEvaluator*>& evaluators,
    vector<ScalarExprEvaluator*>* cloned_evaluators) {
  DCHECK(cloned_evaluators != nullptr);
  DCHECK(cloned_evaluators->empty());
  for (int i = 0; i < evaluators.size(); ++i) {
    ScalarExprEvaluator* cloned_evaluator;
    RETURN_IF_ERROR(evaluators[i]->Clone(pool, state, mem_pool, &cloned_evaluator));
...
bool ScalarExprEvaluator::HasLocalAllocations(
  ...
  for (int i = 0; i < evaluators.size(); ++i) {
    if (evaluators[i]->HasLocalAllocations()) return true;
  }
  return false;
}
...
void ScalarExprEvaluator::FreeLocalAllocations(
    const vector<ScalarExprEvaluator*>& evaluators) {
  for (int i = 0; i < evaluators.size(); ++i) {
    evaluators[i]->FreeLocalAllocations();
  }
}
