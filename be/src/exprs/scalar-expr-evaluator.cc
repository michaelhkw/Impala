...
Status ScalarExprEvaluator::Create(const ScalarExpr& root, RuntimeState* state,
    ObjectPool* pool, MemPool* mem_pool, ScalarExprEvaluator** evaluator) {
  *evaluator = pool->Add(new ScalarExprEvaluator(root, mem_pool));
  if (root.fn_ctx_idx_end_ > 0) {
    (*evaluator)->fn_ctxs_.resize(root.fn_ctx_idx_end_, nullptr);
    (*evaluator)->CreateFnCtxs(state, root);
    DCHECK_EQ((*evaluator)->fn_ctxs_.size(), root.fn_ctx_idx_end_);
    ...
    DCHECK((*evaluator)->fn_ctxs_ptr_ == nullptr);
  }
  (*evaluator)->initialized_ = true;
  return Status::OK();
}

Status ScalarExprEvaluator::Create(const vector<ScalarExpr*>& exprs, RuntimeState* state,
    ObjectPool* pool, MemPool* mem_pool, vector<ScalarExprEvaluator*>* evaluators) {
  for (const ScalarExpr* expr : exprs) {
    ScalarExprEvaluator* evaluator;
    Status status = Create(*expr, state, pool, mem_pool, &evaluator);
...
Status ScalarExprEvaluator::Open(
    const vector<ScalarExprEvaluator*>& evaluators, RuntimeState* state) {
...
Status ScalarExprEvaluator::Clone(ObjectPool* pool, RuntimeState* state,
  ...
  DCHECK(initialized_);
  DCHECK(opened_);
  *cloned_evaluator = pool->Add(new ScalarExprEvaluator(root_, mem_pool));
  for (int i = 0; i < fn_ctxs_.size(); ++i) {
  ...
  (*cloned_evaluator)->initialized_ = true;
  (*cloned_evaluator)->opened_ = true;
  (*cloned_evaluator)->output_scale_ = output_scale_;
  return root_.OpenEvaluator(FunctionContext::THREAD_LOCAL, state, *cloned_evaluator);
}
