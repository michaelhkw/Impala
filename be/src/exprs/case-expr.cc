...
CaseExpr::CaseExpr(const TExprNode& node, int fn_ctx_idx)
...
Status CaseExpr::OpenEvaluator(RuntimeState* state, ScalarExprEvaluator* evaluator,
  ...
  RETURN_IF_ERROR(AllocateAnyVal(state, evaluator->mem_pool(), case_val_type,
  ...
  RETURN_IF_ERROR(AllocateAnyVal(state, evaluator->mem_pool(), when_val_type,
      "Could not allocate expression value", &case_state->when_val));
  return Status::OK();
}

void CaseExpr::CloseEvaluator(RuntimeState* state, ScalarExprEvaluator* evaluator,
    FunctionContext::FunctionStateScope scope) const {
...
void CaseExpr::GetChildVal(int child_idx, ScalarExprEvaluator* evaluator,
    const TupleRow* row, AnyVal* dst) const {
  ScalarExpr* child = GetChild(child_idx);
  switch (child->type().type) {
    case TYPE_BOOLEAN:
      *reinterpret_cast<BooleanVal*>(dst) = child->GetBooleanVal(evaluator, row);
      break;
    case TYPE_TINYINT:
      *reinterpret_cast<TinyIntVal*>(dst) = child->GetTinyIntVal(evaluator, row);
      break;
    case TYPE_SMALLINT:
      *reinterpret_cast<SmallIntVal*>(dst) = child->GetSmallIntVal(evaluator, row);
      break;
    case TYPE_INT:
      *reinterpret_cast<IntVal*>(dst) = child->GetIntVal(evaluator, row);
      break;
    case TYPE_BIGINT:
      *reinterpret_cast<BigIntVal*>(dst) = child->GetBigIntVal(evaluator, row);
      break;
    case TYPE_FLOAT:
      *reinterpret_cast<FloatVal*>(dst) = child->GetFloatVal(evaluator, row);
      break;
    case TYPE_DOUBLE:
      *reinterpret_cast<DoubleVal*>(dst) = child->GetDoubleVal(evaluator, row);
      break;
    case TYPE_TIMESTAMP:
      *reinterpret_cast<TimestampVal*>(dst) = child->GetTimestampVal(evaluator, row);
      break;
    case TYPE_STRING:
      *reinterpret_cast<StringVal*>(dst) = child->GetStringVal(evaluator, row);
      break;
    case TYPE_DECIMAL:
      *reinterpret_cast<DecimalVal*>(dst) = child->GetDecimalVal(evaluator, row);
      break;
    default:
...
#define CASE_COMPUTE_FN(THEN_TYPE) \
  THEN_TYPE CaseExpr::Get##THEN_TYPE( \
    ...
    if (has_case_expr()) {                               \
      ...
      GetChildVal(0, evaluator, row, state->case_val); \
    } else { \
    ...
    if (state->case_val->is_null) { \
      if (has_else_expr()) { \
        /* Return else value. */ \
        return children()[num_children - 1]->Get##THEN_TYPE(evaluator, row); \
      } else { \
    ...
    for (int i = loop_start; i < loop_end; i += 2) { \
      GetChildVal(i, evaluator, row, state->when_val); \
      if (state->when_val->is_null) continue; \
      if (AnyValEq(children()[0]->type(), state->case_val, state->when_val)) { \
        /* Return then value. */ \
        return GetChild(i + 1)->Get##THEN_TYPE(evaluator, row); \
      } \
    } \
    if (has_else_expr()) { \
      /* Return else value. */ \
      return GetChild(num_children - 1)->Get##THEN_TYPE(evaluator, row); \
    } \
    return THEN_TYPE::null(); \
  }
