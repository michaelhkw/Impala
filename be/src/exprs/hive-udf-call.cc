...
HiveUdfCall::HiveUdfCall(const TExprNode& node, int fn_ctx_idx)
    : ScalarExpr(node, fn_ctx_idx), input_buffer_size_(0) {
...
AnyVal* HiveUdfCall::Evaluate(ScalarExprEvaluator* evaluator, const TupleRow* row) const {
  ...
  for (int i = 0; i < GetNumChildren(); ++i) {
    void* v = evaluator->GetValue(GetChild(i), row);
...
Status HiveUdfCall::Init(RuntimeState* state, const RowDescriptor& row_desc) {
  ...
  RETURN_IF_ERROR(ScalarExpr::Init(state, row_desc));
...
Status HiveUdfCall::OpenEvaluator(RuntimeState* state, ScalarExprEvaluator* evaluator,
  ...
  RETURN_IF_ERROR(AllocateAnyVal(state, evaluator->mem_pool(), type_,
      "Could not allocate JNI output value", &jni_ctx->output_anyval));
  return Status::OK();
}

void HiveUdfCall::CloseEvaluator(RuntimeState* state, ScalarExprEvaluator* evaluator,
    FunctionContext::FunctionStateScope scope) const {
  if (evaluator->opened()) {
    FunctionContext* fn_ctx = evaluator->fn_context(fn_ctx_idx_);
    JniContext* jni_ctx = reinterpret_cast<JniContext*>(
...
TimestampVal HiveUdfCall::GetTimestampVal(
  ...
  return *reinterpret_cast<TimestampVal*>(Evaluate(evaluator, row));
}

DecimalVal HiveUdfCall::GetDecimalVal(
    ScalarExprEvaluator* evaluator, const TupleRow* row) const {
  DCHECK_EQ(type_.type, TYPE_DECIMAL);
  return *reinterpret_cast<DecimalVal*>(Evaluate(evaluator, row));
}
