...
Status AggFnEvaluator::Create(const AggFn& agg_fn, RuntimeState* state, ObjectPool* pool,
  ...
  *result = pool->Add(new AggFnEvaluator(agg_fn, mem_pool, false));
  ...
  for (const ScalarExpr* input_expr : agg_fn.children()) {
    ...
    RETURN_IF_ERROR(
        ScalarExprEvaluator::Create(*input_expr, state, pool, mem_pool, &evaluator));
...
Status AggFnEvaluator::Create(const vector<AggFn*>& agg_fns, RuntimeState* state,
    ObjectPool* pool, MemPool* mem_pool, vector<AggFnEvaluator*>* evaluators) {
  for (const AggFn* agg_fn : agg_fns) {
    AggFnEvaluator* evaluator;
    Status status = AggFnEvaluator::Create(*agg_fn, state, pool, mem_pool, &evaluator);
...
Status AggFnEvaluator::Open(
    const vector<AggFnEvaluator*>& evaluators, RuntimeState* state) {
...
void AggFnEvaluator::ShallowClone(ObjectPool* pool, MemPool* mem_pool,
  ...
  for (const AggFnEvaluator* evaluator : evaluators) {
    ...
    evaluator->ShallowClone(pool, mem_pool, &cloned_evaluator);
...
void AggFnEvaluator::FreeLocalAllocations(const vector<AggFnEvaluator*>& evaluators) {
...
void AggFnEvaluator::Finalize(Tuple* agg_val, Tuple* output_val) {
