...
AggFnEvaluator::AggFnEvaluator(const AggFn* agg_fn, MemPool* mem_pool, bool is_clone)
  : opened_(false),
    ...
    mem_pool_(mem_pool),
    agg_fn_ctx_(nullptr),
    staging_intermediate_val_(nullptr),
    staging_merge_input_val_(nullptr) {
}

AggFnEvaluator::~AggFnEvaluator() {
}

inline const SlotDescriptor* AggFnEvaluator::intermediate_slot_desc() const {
  return agg_fn_->intermediate_slot_desc();
}

inline const ColumnType& AggFnEvaluator::intermediate_type() const {
  return agg_fn_->intermediate_type();
}

Status AggFnEvaluator::Create(ObjectPool* pool, RuntimeState* state, MemPool* mem_pool,
    const AggFn* agg_fn, AggFnEvaluator** result) {
  *result = pool->Add(new AggFnEvaluator(agg_fn, mem_pool, false));
  (*result)->agg_fn_ctx_ = FunctionContextImpl::CreateContext(state, mem_pool,
  ...
  for (const ScalarExpr* input_expr : agg_fn->children()) {
    ...
    RETURN_IF_ERROR(
        ScalarExprEvaluator::Create(pool, state, mem_pool, input_expr, &evaluator));
    (*result)->input_evaluators_.push_back(evaluator);
    DCHECK(evaluator->root() == input_expr);

    AnyVal* staging_input_val;
    RETURN_IF_ERROR(AllocateAnyVal(state, mem_pool, input_expr->type(),
        "Could not allocate aggregate expression input value", &staging_input_val));
    (*result)->staging_input_vals_.push_back(staging_input_val);
  }
  DCHECK_EQ(agg_fn->GetNumChildren(), (*result)->input_evaluators_.size());
  DCHECK_EQ((*result)->staging_input_vals_.size(), (*result)->input_evaluators_.size());

  RETURN_IF_ERROR(AllocateAnyVal(state, mem_pool, agg_fn->intermediate_type(),
      "Could not allocate aggregate expression intermediate value",
      &((*result)->staging_intermediate_val_)));
  RETURN_IF_ERROR(AllocateAnyVal(state, mem_pool, agg_fn->intermediate_type(),
      "Could not allocate aggregate expression merge input value",
      &((*result)->staging_merge_input_val_)));

  if (agg_fn->is_merge()) {
    DCHECK_EQ((*result)->staging_input_vals_.size(), 1)
...
Status AggFnEvaluator::Create(ObjectPool* pool, RuntimeState* state, MemPool* mem_pool,
    const vector<AggFn*>& agg_fns, vector<AggFnEvaluator*>* evaluators) {
  for (const AggFn* agg_fn : agg_fns) {
    AggFnEvaluator* evaluator;
    Status status = AggFnEvaluator::Create(pool, state, mem_pool, agg_fn, &evaluator);
...
Status AggFnEvaluator::Open(RuntimeState* state) {
  ...
  opened_ = true;
  RETURN_IF_ERROR(ScalarExprEvaluator::Open(input_evaluators_, state));
  ...
  // on them).
  vector<AnyVal*> constant_args(input_evaluators_.size(), nullptr);
  for (int i = 0; i < input_evaluators_.size(); ++i) {
    ScalarExprEvaluator* evaluator = input_evaluators_[i];
    RETURN_IF_ERROR(evaluator->GetConstValue(state, agg_fn_->GetChild(i),
        &constant_args[i]));
  }
...
Status AggFnEvaluator::Open(vector<AggFnEvaluator*>& evaluators, RuntimeState* state) {
  for (AggFnEvaluator* evaluator : evaluators) RETURN_IF_ERROR(evaluator->Open(state));
  return Status::OK();
}

void AggFnEvaluator::Close(RuntimeState* state) {
  if (!is_clone_) ScalarExprEvaluator::Close(input_evaluators_, state);
  FreeLocalAllocations();
  agg_fn_ctx_->impl()->Close();
  input_evaluators_.clear();
}

void AggFnEvaluator::Close(vector<AggFnEvaluator*>& evaluators, RuntimeState* state) {
  for (AggFnEvaluator* evaluator : evaluators) evaluator->Close(state);
}
...
void AggFnEvaluator::Init(Tuple* dst) {
  ...
  for (ScalarExprEvaluator* input_evaluator : input_evaluators_) {
    DCHECK(input_evaluator->opened());
  }
  ...
  reinterpret_cast<InitFn>(agg_fn_->init_fn_)(agg_fn_ctx_, staging_intermediate_val_);
...
void AggFnEvaluator::Update(const TupleRow* row, Tuple* dst, void* fn) {
  ...
  for (int i = 0; i < input_evaluators_.size(); ++i) {
    void* src_slot = input_evaluators_[i]->GetValue(row);
    DCHECK(input_evaluators_[i]->root() == agg_fn_->GetChild(i));
  ...
  switch (input_evaluators_.size()) {
    ...
    case 7:
      ...
      break;
    case 8:
      reinterpret_cast<UpdateFn8>(fn)(agg_fn_ctx_,
          *staging_input_vals_[0], *staging_input_vals_[1],
          *staging_input_vals_[2], *staging_input_vals_[3],
          *staging_input_vals_[4], *staging_input_vals_[5],
...
void AggFnEvaluator::Merge(Tuple* src, Tuple* dst) {
  ...
  reinterpret_cast<UpdateFn1>(agg_fn_->merge_fn_)(agg_fn_ctx_,
...
void AggFnEvaluator::SerializeOrFinalize(Tuple* src,
  ...
  switch (dst_slot_desc->type().type) {
    case TYPE_BOOLEAN: {
      ...
      BooleanVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    case TYPE_TINYINT: {
      typedef TinyIntVal(*Fn)(FunctionContext*, AnyVal*);
      TinyIntVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    case TYPE_SMALLINT: {
      typedef SmallIntVal(*Fn)(FunctionContext*, AnyVal*);
      SmallIntVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    case TYPE_INT: {
      typedef IntVal(*Fn)(FunctionContext*, AnyVal*);
      IntVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    case TYPE_BIGINT: {
      typedef BigIntVal(*Fn)(FunctionContext*, AnyVal*);
      BigIntVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    case TYPE_FLOAT: {
      typedef FloatVal(*Fn)(FunctionContext*, AnyVal*);
      FloatVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    case TYPE_DOUBLE: {
      typedef DoubleVal(*Fn)(FunctionContext*, AnyVal*);
      DoubleVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    case TYPE_STRING:
    case TYPE_VARCHAR: {
      ...
      StringVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    case TYPE_DECIMAL: {
      typedef DecimalVal(*Fn)(FunctionContext*, AnyVal*);
      DecimalVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    case TYPE_TIMESTAMP: {
      typedef TimestampVal(*Fn)(FunctionContext*, AnyVal*);
      TimestampVal v = reinterpret_cast<Fn>(fn)(agg_fn_ctx_, staging_intermediate_val_);
      SetDstSlot(&v, dst_slot_desc, dst);
      break;
    }
    default:
...
void AggFnEvaluator::Clone(ObjectPool* pool, MemPool* mem_pool,
    AggFnEvaluator** cloned_evaluator) const {
  DCHECK(opened_);
  *cloned_evaluator = pool->Add(new AggFnEvaluator(agg_fn_, mem_pool, true));
  (*cloned_evaluator)->agg_fn_ctx_ = agg_fn_ctx_->impl()->Clone(mem_pool);
  DCHECK_EQ((*cloned_evaluator)->input_evaluators_.size(), 0);
  (*cloned_evaluator)->input_evaluators_ = input_evaluators_;
  (*cloned_evaluator)->staging_input_vals_ = staging_input_vals_;
  (*cloned_evaluator)->staging_intermediate_val_ = staging_intermediate_val_;
  (*cloned_evaluator)->staging_merge_input_val_ = staging_merge_input_val_;
  (*cloned_evaluator)->opened_ = true;
}

void AggFnEvaluator::Clone(ObjectPool* pool, MemPool* mem_pool,
    const vector<AggFnEvaluator*>& evaluators,
    vector<AggFnEvaluator*>* cloned_evaluators) {
  for (const AggFnEvaluator* evaluator : evaluators) {
    AggFnEvaluator* cloned_evaluator;
    evaluator->Clone(pool, mem_pool, &cloned_evaluator);
    cloned_evaluators->push_back(cloned_evaluator);
  }
}

void AggFnEvaluator::FreeLocalAllocations() {
  ScalarExprEvaluator::FreeLocalAllocations(input_evaluators_);
  agg_fn_ctx_->impl()->FreeLocalAllocations();
}

void AggFnEvaluator::FreeLocalAllocations(vector<AggFnEvaluator*>& evaluators) {
  for (AggFnEvaluator* evaluator : evaluators) evaluator->FreeLocalAllocations();
}

void AggFnEvaluator::Add(const TupleRow* row, Tuple* dst) {
  agg_fn_ctx_->impl()->IncrementNumUpdates();
  Update(row, dst, agg_fn_->merge_or_update_fn());
}

void AggFnEvaluator::Remove(const TupleRow* row, Tuple* dst) {
...
void AggFnEvaluator::GetValue(Tuple* src, Tuple* dst) {
  SerializeOrFinalize(src, agg_fn_->output_slot_desc(), dst,
      agg_fn_->get_value_fn());
}

void AggFnEvaluator::Init(const std::vector<AggFnEvaluator*>& evaluators, Tuple* dst) {
  for (int i = 0; i < evaluators.size(); ++i) {
    evaluators[i]->Init(dst);
  }
}

void AggFnEvaluator::Add(const std::vector<AggFnEvaluator*>& evaluators,
    const TupleRow* src, Tuple* dst) {
  for (int i = 0; i < evaluators.size(); ++i) {
    evaluators[i]->Add(src, dst);
  }
}

void AggFnEvaluator::Remove(const std::vector<AggFnEvaluator*>& evaluators,
    const TupleRow* src, Tuple* dst) {
  for (int i = 0; i < evaluators.size(); ++i) {
    evaluators[i]->Remove(src, dst);
  }
}

void AggFnEvaluator::Serialize(const std::vector<AggFnEvaluator*>& evaluators,
    Tuple* dst) {
  for (int i = 0; i < evaluators.size(); ++i) {
    evaluators[i]->Serialize(dst);
  }
}

void AggFnEvaluator::GetValue(const std::vector<AggFnEvaluator*>& evaluators,
    Tuple* src, Tuple* dst) {
  for (int i = 0; i < evaluators.size(); ++i) {
    evaluators[i]->GetValue(src, dst);
  }
}

void AggFnEvaluator::Finalize(const std::vector<AggFnEvaluator*>& evaluators,
    Tuple* src, Tuple* dst) {
  for (int i = 0; i < evaluators.size(); ++i) {
    evaluators[i]->Finalize(src, dst);
  }
}
