...
ScalarFnCall::ScalarFnCall(const TExprNode& node, int fn_ctx_idx)
  : ScalarExpr(node, fn_ctx_idx),
...
Status ScalarFnCall::Init(RuntimeState* state, const RowDescriptor& desc) {
  ...
  RETURN_IF_ERROR(ScalarExpr::Init(state, desc));
...
Status ScalarFnCall::OpenEvaluator(RuntimeState* state, ScalarExprEvaluator* evaluator,
    FunctionContext::FunctionStateScope scope) const {
  // Opens and inits children
  RETURN_IF_ERROR(ScalarExpr::OpenEvaluator(state, evaluator, scope));
  FunctionContext* fn_ctx = evaluator->fn_context(fn_ctx_idx_);
  ...
  if (need_interp) {
    ...
    for (int i = 0; i < NumFixedArgs(); ++i) {
      AnyVal* input_val;
      RETURN_IF_ERROR(AllocateAnyVal(state, evaluator->mem_pool(), children_[i]->type(),
  ...
  // If 'evaluator' was cloned, the constant values were copied from the parent.
  if (scope == FunctionContext::FRAGMENT_LOCAL) {
    vector<AnyVal*> constant_args;
    for (const ScalarExpr* child : children()) {
      AnyVal* const_val;
      RETURN_IF_ERROR(evaluator->GetConstValue(state, child, &const_val));
    ...
    if (fn_.name.function_name == "round" && type_.type == TYPE_DOUBLE) {
      ...
      if (scale_arg != nullptr) evaluator->output_scale_ = scale_arg->val;
    }
  }
...
void ScalarFnCall::CloseEvaluator(RuntimeState* state, ScalarExprEvaluator* evaluator,
    FunctionContext::FunctionStateScope scope) const {
  ...
  ScalarExpr::CloseEvaluator(state, evaluator, scope);
}
...
Status ScalarFnCall::GetCodegendComputeFn(LlvmCodeGen* codegen, Function** fn) {
  ...
  *fn = CreateIrFunctionPrototype(codegen, fn_name.str(), &args);
  ...
  Value* evaluator_gep = builder.CreateStructGEP(NULL, evaluator, 1, "evaluator_gep");
  Value* fn_ctxs_base = builder.CreateLoad(evaluator_gep, "fn_ctxs_base");
  ...
  for (int i = 0; i < GetNumChildren(); ++i) {
    ...
    child_fn_args.push_back(evaluator);
...
void ScalarFnCall::EvaluateNonConstantChildren(
  ...
  for (pair<ScalarExpr*, AnyVal*> child : fn_ctx->impl()->non_constant_args()) {
    void* val = evaluator->GetValue(child.first, row);
    AnyValUtil::SetAnyVal(val, child.first->type(), child.second);
  }
}

template<typename RETURN_TYPE>
RETURN_TYPE ScalarFnCall::InterpretEval(ScalarExprEvaluator* evaluator,
    const TupleRow* row) const {
  DCHECK(scalar_fn_ != NULL) << DebugString();
  FunctionContext* fn_ctx = evaluator->fn_context(fn_ctx_idx_);
  vector<AnyVal*>* input_vals = fn_ctx->impl()->staging_input_vals();
  EvaluateNonConstantChildren(evaluator, row);
...
BooleanVal ScalarFnCall::GetBooleanVal(
  ...
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<BooleanVal>(evaluator, row);
  BooleanWrapper fn = reinterpret_cast<BooleanWrapper>(scalar_fn_wrapper_);
  return fn(evaluator, row);
}
...
TinyIntVal ScalarFnCall::GetTinyIntVal(
  ...
  DCHECK(evaluator != NULL);
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<TinyIntVal>(evaluator, row);
...
SmallIntVal ScalarFnCall::GetSmallIntVal(
  ...
  DCHECK(evaluator != NULL);
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<SmallIntVal>(evaluator, row);
...
IntVal ScalarFnCall::GetIntVal(
  ...
  DCHECK(evaluator != NULL);
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<IntVal>(evaluator, row);
...
BigIntVal ScalarFnCall::GetBigIntVal(
  ...
  DCHECK(evaluator != NULL);
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<BigIntVal>(evaluator, row);
...
FloatVal ScalarFnCall::GetFloatVal(
  ...
  DCHECK(evaluator != NULL);
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<FloatVal>(evaluator, row);
...
DoubleVal ScalarFnCall::GetDoubleVal(
  ...
  DCHECK(evaluator != NULL);
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<DoubleVal>(evaluator, row);
...
StringVal ScalarFnCall::GetStringVal(
  ...
  DCHECK(evaluator != NULL);
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<StringVal>(evaluator, row);
...
TimestampVal ScalarFnCall::GetTimestampVal(
  ...
  DCHECK(evaluator != NULL);
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<TimestampVal>(evaluator, row);
...
DecimalVal ScalarFnCall::GetDecimalVal(
  ...
  DCHECK(evaluator != NULL);
  if (scalar_fn_wrapper_ == NULL) return InterpretEval<DecimalVal>(evaluator, row);
