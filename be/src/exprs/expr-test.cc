...
void TestSingleLiteralConstruction(
  ...
  EXPECT_OK(ScalarExprEvaluator::Create(*expr, &state, &pool, &mem_pool, &evaluator));
  EXPECT_OK(evaluator->Open(&state));
  EXPECT_EQ(0, RawValue::Compare(evaluator->GetValue(nullptr), &value, type))
...
TEST_F(ExprTest, NullLiteral) {
  for (int type = TYPE_BOOLEAN; type != TYPE_DATE; ++type) {
    ...
    EXPECT_OK(ScalarExprEvaluator::Create(expr, &state, &pool, &mem_pool, &evaluator));
    EXPECT_OK(evaluator->Open(&state));
    EXPECT_TRUE(evaluator->GetValue(nullptr) == nullptr);
    evaluator->Close(&state);
    state.ReleaseResources();
  }
}
