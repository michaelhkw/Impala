...
Status ScalarExpr::Create(const TExpr& texpr, const RowDescriptor& row_desc,
  ...
  RETURN_IF_ERROR(CreateNode(texpr.nodes[0], pool, scalar_expr));
  RETURN_IF_ERROR(Expr::CreateTree(texpr, pool, *scalar_expr));
  int fn_ctx_idx = 0;
  (*scalar_expr)->AssignFnCtxIdx(&fn_ctx_idx);
  return (*scalar_expr)->Init(row_desc, state);
}
...
Status ScalarExpr::Create(const vector<TExpr>& texprs, const RowDescriptor& row_desc,
  ...
  for (const TExpr& texpr: texprs) {
    ScalarExpr* expr;
    Status status = Create(texpr, row_desc, state, pool, &expr);
    // Add 'expr' to the vector before returning error status if any.
...
Status ScalarExpr::OpenEvaluator(FunctionContext::FunctionStateScope scope,
  ...
  for (int i = 0; i < children_.size(); ++i) {
    RETURN_IF_ERROR(children_[i]->OpenEvaluator(scope, state, evaluator));
  }
  return Status::OK();
}
...
void ScalarExpr::CloseEvaluator(FunctionContext::FunctionStateScope scope,
    RuntimeState* state, ScalarExprEvaluator* evaluator) const {
  for (ScalarExpr* child : children_) child->CloseEvaluator(scope, state, evaluator);
}
