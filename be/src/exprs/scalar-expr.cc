...
ScalarExpr::ScalarExpr(const ColumnType& type, bool is_constant, int fn_ctx_idx)
  : Expr(type, is_constant),
    ir_compute_fn_(nullptr),
...
ScalarExpr::ScalarExpr(const TExprNode& node, int fn_ctx_idx)
  : Expr(node),
    ir_compute_fn_(nullptr),
    fn_ctx_idx_(fn_ctx_idx),
    fn_ctx_idx_start_(0),
...
Status ScalarExpr::Create(ObjectPool* pool, RuntimeState* state,
    const RowDescriptor& row_desc, const TExpr& texpr, ScalarExpr** scalar_expr) {
  int fn_ctx_idx = 0;
  RETURN_IF_ERROR(CreateNode(pool, texpr.nodes[0], scalar_expr, &fn_ctx_idx));
  RETURN_IF_ERROR(Expr::CreateTree(pool, texpr, *scalar_expr, &fn_ctx_idx));
  (*scalar_expr)->fn_ctx_idx_end_ = fn_ctx_idx;
  return (*scalar_expr)->Init(state, row_desc);
}

Status ScalarExpr::Create(ObjectPool* pool, RuntimeState* state,
    const RowDescriptor& row_desc, const vector<TExpr>& texprs,
    vector<ScalarExpr*>* exprs) {
  exprs->clear();
  for (const TExpr& texpr: texprs) {
    ScalarExpr* expr;
    Status status = Create(pool, state, row_desc, texpr, &expr);
...
Status ScalarExpr::CreateNode(ObjectPool* pool, const TExprNode& texpr_node,
    ScalarExpr** expr, int* fn_ctx_idx) {
  switch (texpr_node.node_type) {
    ...
    case TExprNodeType::CASE_EXPR:
      ...
      *expr = pool->Add(new CaseExpr(texpr_node, *fn_ctx_idx));
      ++(*fn_ctx_idx);
      return Status::OK();
    case TExprNodeType::COMPOUND_PRED:
      if (texpr_node.fn.name.function_name == "and") {
        ...
        *expr = pool->Add(new ScalarFnCall(texpr_node, *fn_ctx_idx));
        ++(*fn_ctx_idx);
      }
      return Status::OK();
    ...
    case TExprNodeType::FUNCTION_CALL:
      ...
      if (texpr_node.fn.name.function_name == "if") {
        ...
        *expr = pool->Add(new HiveUdfCall(texpr_node, *fn_ctx_idx));
        ++(*fn_ctx_idx);
      } else {
        *expr = pool->Add(new ScalarFnCall(texpr_node, *fn_ctx_idx));
        ++(*fn_ctx_idx);
...
Status ScalarExpr::OpenEvaluator(RuntimeState* state, ScalarExprEvaluator* evaluator,
    FunctionContext::FunctionStateScope scope) const {
  for (int i = 0; i < children_.size(); ++i) {
    RETURN_IF_ERROR(children_[i]->OpenEvaluator(state, evaluator, scope));
  }
  return Status::OK();
}

void ScalarExpr::CloseEvaluator(RuntimeState* state, ScalarExprEvaluator* evaluator,
    FunctionContext::FunctionStateScope scope) const {
  for (ScalarExpr* child : children_) child->CloseEvaluator(state, evaluator, scope);
}
...
void ScalarExpr::Close(vector<ScalarExpr*>& exprs) {
...
Status ScalarExpr::Init(RuntimeState* state, const RowDescriptor& row_desc) {
...
Function* ScalarExpr::CreateIrFunctionPrototype(LlvmCodeGen* codegen,
     const string& name, Value* (*args)[2]) {
  ...
  prototype.AddArgument(
      LlvmCodeGen::NamedVariable(
          "evaluator", codegen->GetPtrType(ScalarExprEvaluator::LLVM_CLASS_NAME)));
  prototype.AddArgument(
