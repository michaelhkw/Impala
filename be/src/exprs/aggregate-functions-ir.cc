...
DecimalVal AggregateFunctions::DecimalAvgGetValue(FunctionContext* ctx,
  ...
  if (!ctx->impl()->GetConstFnAttr(FunctionContextImpl::DECIMAL_V2)) {
    // For DECIMAL_V1, we set the return type's scale to the input scale. The scale of
    // the accumulated sum must be the same as the scale of the return type.
    DCHECK_EQ(sum_scale, output_scale);
  }
  bool is_nan = false;
  bool overflow = false;
