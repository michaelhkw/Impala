...
AggFn::AggFn(const TExprNode& tnode)
  : Expr(tnode),
    is_merge_(tnode.agg_expr.is_merge_agg),
    intermediate_slot_desc_(nullptr),
    output_slot_desc_(nullptr),
...
Status AggFn::Init(RuntimeState* state, const RowDescriptor& row_desc,
    const SlotDescriptor* intermediate_slot_desc,
    const SlotDescriptor* output_slot_desc) {
  // Initialize all children (i.e. input exprs to this aggregate expr).
  for (ScalarExpr* input_expr : children()) {
    RETURN_IF_ERROR(input_expr->Init(state, row_desc));
  }
  ...
  DCHECK(output_slot_desc != nullptr);
...
Status AggFn::Create(ObjectPool* pool, RuntimeState* state, const RowDescriptor& row_desc,
    const SlotDescriptor* intermediate_slot_desc, const SlotDescriptor* output_slot_desc,
    const TExpr& texpr, AggFn** agg_fn) {
  ...
  int fn_ctx_idx = 0;
  *agg_fn = pool->Add(new AggFn(texpr_node));
  RETURN_IF_ERROR(Expr::CreateTree(pool, texpr, *agg_fn, &fn_ctx_idx));
  return (*agg_fn)->Init(state, row_desc, intermediate_slot_desc, output_slot_desc);
}
...
void AggFn::Close(vector<AggFn*>& exprs) {
