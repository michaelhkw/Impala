...
Status AggFn::Create(const TExpr& texpr, const RowDescriptor& row_desc,
  ...
  *agg_fn = pool->Add(new AggFn(texpr_node, intermediate_slot_desc, output_slot_desc));
  RETURN_IF_ERROR(Expr::CreateTree(texpr, pool, *agg_fn));
  for (ScalarExpr* input_expr : (*agg_fn)->children()) {
    int fn_ctx_idx = 0;
    input_expr->AssignFnCtxIdx(&fn_ctx_idx);
  }
  return (*agg_fn)->Init(row_desc, state);
}
