...
class CodegenCallGraph {
 public:
  ...
  inline const boost::unordered_set<std::string>* GetCallees(
      const std::string& fn_name) const {
    DCHECK(inited_);
    CallGraph::const_iterator iter = call_graph_.find(fn_name);
    if (LIKELY(iter != call_graph_.end())) {
      return &iter->second;
    } else {
      return NULL;
    }
  ...
  inline const boost::unordered_set<std::string>& fns_referenced_by_gv() const {
    DCHECK(inited_);
