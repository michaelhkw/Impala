...
Constant* LlvmCodeGen::GetIntConstant(PrimitiveType type, int64_t val) {
  switch (type) {
    case TYPE_TINYINT:
...
Constant* LlvmCodeGen::GetIntConstant(int num_bytes, int64_t val) {
...
Constant* LlvmCodeGen::ConstantToGVPtr(Type* type, Constant* ir_constant,
  ...
  GlobalVariable* gv = new GlobalVariable(*module_, type, true,
      GlobalValue::ExternalLinkage, ir_constant, name);
  gv->setInitializer(ir_constant);
