...
/// threads are configurable via FLAGS_rpc_thread. Incoming events are passed
...
class RpcMgr {
 public:
  ...
  /// Start the acceptor threads which listen on 'service_port', making Kudu RPC
  /// services available. Before this method is called, remote clients will get a
  /// 'connection refused' error when trying to invoke an RPC on this machine.
  Status StartServices(int service_port) WARN_UNUSED_RESULT;
  ...
  /// it in 'proxy'. 'P' must descend from kudu::rpc::ServiceIf.
  template <typename P>
