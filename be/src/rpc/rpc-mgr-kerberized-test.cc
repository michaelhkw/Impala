...
TEST_P(RpcMgrKerberizedTest, AuthorizationFail) {
  ...
  // Switch over to a credential cache which only contains the dummy credential.
  // Kinit done in InitAuth() uses a different credential cache.
  ...
  ASSERT_OK(rpc_mgr_.GetProxy<ScanMemServiceProxy>(krpc_address_, &scan_proxy));
...
int main(int argc, char** argv) {
  ...
  DCHECK(status.ok());
  principal = Substitute("impala-test/$0", ip);
  realm = "KRBTEST.COM";
