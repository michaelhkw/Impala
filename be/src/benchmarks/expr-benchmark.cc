...
static Status PrepareSelectList(
    const TExecRequest& request, ScalarExprEvaluator** evaluator) {
  ...
  ScalarExpr* expr;
  RETURN_IF_ERROR(ScalarExpr::Create(&pool, state, RowDescriptor(), texprs[0], &expr));
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(&pool, state, &mem_pool, expr, evaluator));
  return Status::OK();
}
...
static TestData* GenerateBenchmarkExprs(const string& query, bool codegen) {
  ...
  ABORT_IF_ERROR(PrepareSelectList(request, &test_data->evaluator));
  return test_data;
}
...
void BenchmarkQueryFn(int batch_size, void* d) {
  ...
  for (int i = 0; i < batch_size; ++i) {
    for (int n = 0; n < ITERATIONS; ++n) {
      void* value = data->evaluator->GetValue(NULL);
