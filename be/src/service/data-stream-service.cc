...
DEFINE_int32(datastream_service_num_svc_threads, 0, "Number of datastream service "
    ...
    "cores.");
DEFINE_int32(datastream_service_callback_period_ms, 60 * MILLIS_PER_SEC, "Period in "
    "milliseconds in which the callback to drain the service threads' cache is invoked");
...
Status DataStreamService::Init(RpcMgr* rpc_mgr) {
  return rpc_mgr->RegisterService(num_svc_threads_,
      ...
      boost::bind(&DataStreamService::DrainQueue, this),
      FLAGS_datastream_service_callback_period_ms);
}
