...
Status TopNNode::Prepare(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(output_tuple_exprs_, state, pool_,
      expr_mem_pool(), &output_tuple_expr_evaluators_));
  AddEvaluatorsToFree(output_tuple_expr_evaluators_);
  tuple_row_less_than_.reset(
      new TupleRowComparator(ordering_exprs_, is_asc_order_, nulls_first_));
...
Status TopNNode::Open(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Open(output_tuple_expr_evaluators_, state));
...
void TopNNode::Close(RuntimeState* state) {
  ...
  ScalarExprEvaluator::Close(output_tuple_expr_evaluators_, state);
