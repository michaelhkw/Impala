...
class HdfsParquetTableWriter::BaseColumnWriter {
 public:
  ...
  BaseColumnWriter(HdfsParquetTableWriter* parent, ScalarExprEvaluator* expr_evaluator,
  ...
  const ColumnType& type() const { return expr_evaluator_->root()->type(); }
 ...
 protected:
  ...
  ScalarExprEvaluator* expr_evaluator_;
...
class HdfsParquetTableWriter::ColumnWriter :
 ...
 public:
  ColumnWriter(HdfsParquetTableWriter* parent, ScalarExprEvaluator* evaluator,
      const THdfsCompression::type& codec)
    : BaseColumnWriter(parent, evaluator, codec),
      num_values_since_dict_size_check_(0),
      plain_encoded_value_size_(
          ParquetPlainEncoder::EncodedByteSize(evaluator->root()->type())),
      page_stats_(plain_encoded_value_size_),
      row_group_stats_(plain_encoded_value_size_) {
    DCHECK_NE(evaluator->root()->type().type, TYPE_BOOLEAN);
...
class HdfsParquetTableWriter::BoolColumnWriter :
    public HdfsParquetTableWriter::BaseColumnWriter {
 public:
  BoolColumnWriter(HdfsParquetTableWriter* parent, ScalarExprEvaluator* evaluator,
      const THdfsCompression::type& codec)
    : BaseColumnWriter(parent, evaluator, codec), page_stats_(-1), row_group_stats_(-1) {
    DCHECK_EQ(evaluator->root()->type().type, TYPE_BOOLEAN);
...
inline Status HdfsParquetTableWriter::BaseColumnWriter::AppendRow(TupleRow* row) {
  ++num_values_;
  void* value = expr_evaluator_->GetValue(row);
...
HdfsParquetTableWriter::HdfsParquetTableWriter(HdfsTableSink* parent, RuntimeState* state,
  ...
  : HdfsTableWriter(parent, state, output, part_desc, table_desc, output_expr_evaluators),
...
Status HdfsParquetTableWriter::Init() {
  ...
  for (int i = 0; i < columns_.size(); ++i) {
    ...
    const ColumnType& type = output_expr_evaluators_[i]->root()->type();
    switch (type.type) {
      case TYPE_BOOLEAN:
        writer = new BoolColumnWriter(this, output_expr_evaluators_[i], codec);
        break;
      case TYPE_TINYINT:
        writer = new ColumnWriter<int8_t>(this, output_expr_evaluators_[i], codec);
        break;
      case TYPE_SMALLINT:
        writer = new ColumnWriter<int16_t>(this, output_expr_evaluators_[i], codec);
        break;
      case TYPE_INT:
        writer = new ColumnWriter<int32_t>(this, output_expr_evaluators_[i], codec);
        break;
      case TYPE_BIGINT:
        writer = new ColumnWriter<int64_t>(this, output_expr_evaluators_[i], codec);
        break;
      case TYPE_FLOAT:
        writer = new ColumnWriter<float>(this, output_expr_evaluators_[i], codec);
        break;
      case TYPE_DOUBLE:
        writer = new ColumnWriter<double>(this, output_expr_evaluators_[i], codec);
        break;
      case TYPE_TIMESTAMP:
        writer = new ColumnWriter<TimestampValue>(
            this, output_expr_evaluators_[i], codec);
        break;
      case TYPE_VARCHAR:
      case TYPE_STRING:
      case TYPE_CHAR:
        writer = new ColumnWriter<StringValue>(this, output_expr_evaluators_[i], codec);
        break;
      case TYPE_DECIMAL:
        switch (output_expr_evaluators_[i]->root()->type().GetByteSize()) {
          case 4:
  ...
  for (int i = 0; i < columns_.size(); ++i) {
    ...
    if (type.type == TYPE_DECIMAL) {
      ...
      node.__set_type_length(
          ParquetPlainEncoder::DecimalSize(output_expr_evaluators_[i]->root()->type()));
      node.__set_scale(output_expr_evaluators_[i]->root()->type().scale);
      node.__set_precision(output_expr_evaluators_[i]->root()->type().precision);
