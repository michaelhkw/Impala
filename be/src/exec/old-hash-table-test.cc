...
class OldHashTableTest : public testing::Test {
 ...
 protected:
  ...
  vector<ScalarExprEvaluator*> build_expr_evaluators_;
  vector<ScalarExpr*> probe_exprs_;
  vector<ScalarExprEvaluator*> probe_expr_evaluators_;
  ...
  virtual void SetUp() {
    ...
    ASSERT_OK(build_expr->Init(nullptr, desc));
    build_exprs_.push_back(build_expr);
    ASSERT_OK(ScalarExprEvaluator::Create(&pool_, nullptr, &mem_pool_, build_exprs_,
        &build_expr_evaluators_));
    ASSERT_OK(ScalarExprEvaluator::Open(build_expr_evaluators_, nullptr));

    ScalarExpr* probe_expr = pool_.Add(new SlotRef(TYPE_INT, 0));
    ASSERT_OK(probe_expr->Init(nullptr, desc));
    probe_exprs_.push_back(probe_expr);
    ASSERT_OK(ScalarExprEvaluator::Create(&pool_, nullptr, &mem_pool_, probe_exprs_,
        &probe_expr_evaluators_));
    ASSERT_OK(ScalarExprEvaluator::Open(probe_expr_evaluators_, nullptr));
  }

  virtual void TearDown() {
    ScalarExprEvaluator::Close(build_expr_evaluators_, nullptr);
    ScalarExprEvaluator::Close(probe_expr_evaluators_, nullptr);
  ...
  void FullScan(OldHashTable* table, int min, int max, bool all_unique,
    ...
    while (iter != table->End()) {
      ...
      int32_t val = *reinterpret_cast<int32_t*>(build_expr_evaluators_[0]->GetValue(row));
      EXPECT_GE(val, min);
      EXPECT_LT(val, max);
  ...
  void ValidateMatch(TupleRow* probe_row, TupleRow* build_row) {
    ...
    int32_t build_val =
        *reinterpret_cast<int32_t*>(build_expr_evaluators_[0]->GetValue(probe_row));
    int32_t probe_val =
        *reinterpret_cast<int32_t*>(probe_expr_evaluators_[0]->GetValue(build_row));
...
TEST_F(OldHashTableTest, SetupTest) {
  ...
  int32_t* val_row1 =
      reinterpret_cast<int32_t*>(build_expr_evaluators_[0]->GetValue(build_row1));
  EXPECT_EQ(*val_row1, 1);
  int32_t* val_row2 =
      reinterpret_cast<int32_t*>(build_expr_evaluators_[0]->GetValue(build_row2));
  EXPECT_EQ(*val_row2, 2);
  int32_t* val_row3 =
      reinterpret_cast<int32_t*>(probe_expr_evaluators_[0]->GetValue(probe_row3));
  EXPECT_EQ(*val_row3, 3);
  int32_t* val_row4 =
      reinterpret_cast<int32_t*>(probe_expr_evaluators_[0]->GetValue(probe_row4));
