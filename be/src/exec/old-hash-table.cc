...
Status OldHashTable::Init(ObjectPool* pool, RuntimeState* state) {
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(pool, state, mem_pool_.get(),
      build_exprs_, &build_expr_evaluators_));
  DCHECK_EQ(build_exprs_.size(), build_expr_evaluators_.size());
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(pool, state, mem_pool_.get(),
      probe_exprs_, &probe_expr_evaluators_));
  DCHECK_EQ(probe_exprs_.size(), probe_expr_evaluators_.size());
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(pool, state, mem_pool_.get(),
      filter_exprs_, &filter_expr_evaluators_));
  DCHECK_EQ(filter_exprs_.size(), filter_expr_evaluators_.size());
  return Status::OK();
}
...
Status OldHashTable::Open(RuntimeState* state) {
  RETURN_IF_ERROR(ScalarExprEvaluator::Open(build_expr_evaluators_, state));
  DCHECK_EQ(build_exprs_.size(), build_expr_evaluators_.size());
  RETURN_IF_ERROR(ScalarExprEvaluator::Open(probe_expr_evaluators_, state));
  DCHECK_EQ(probe_exprs_.size(), probe_expr_evaluators_.size());
  RETURN_IF_ERROR(ScalarExprEvaluator::Open(filter_expr_evaluators_, state));
  DCHECK_EQ(filter_exprs_.size(), filter_expr_evaluators_.size());
  return Status::OK();
}
...
void OldHashTable::Close(RuntimeState* state) {
  ...
  ScalarExprEvaluator::Close(build_expr_evaluators_, state);
  ScalarExprEvaluator::Close(probe_expr_evaluators_, state);
  ScalarExprEvaluator::Close(filter_expr_evaluators_, state);
  mem_pool_->FreeAll();
...
void OldHashTable::FreeLocalAllocations() {
  ScalarExprEvaluator::FreeLocalAllocations(build_expr_evaluators_);
  ScalarExprEvaluator::FreeLocalAllocations(probe_expr_evaluators_);
  ScalarExprEvaluator::FreeLocalAllocations(filter_expr_evaluators_);
}

bool OldHashTable::EvalRow(
    TupleRow* row, const vector<ScalarExprEvaluator*>& evaluators) {
  bool has_null = false;
  for (int i = 0; i < evaluators.size(); ++i) {
    void* loc = expr_values_buffer_ + expr_values_buffer_offsets_[i];
    void* val = evaluators[i]->GetValue(row);
    if (val == NULL) {
...
int OldHashTable::AddBloomFilters() {
  ...
  while (iter != End()) {
    ...
    for (int i = 0; i < filters_.size(); ++i) {
      ...
      void* e = filter_expr_evaluators_[i]->GetValue(row);
...
Function* OldHashTable::CodegenEvalTupleRow(LlvmCodeGen* codegen, bool build) {
  ...
  Function* get_expr_evaluator_fn = codegen->GetFunction(fn_name, false);
  DCHECK(get_expr_evaluator_fn != NULL);
  ...
  if (!exprs.empty()) {
    // Load &build_expr_evaluators_[0] / &probe_expr_evaluators_[0]
    Value* evaluator_vector = codegen->CodegenCallFunction(&builder, build ?
        IRFunction::OLD_HASH_TABLE_GET_BUILD_EXPR_EVALUATORS :
        IRFunction::OLD_HASH_TABLE_GET_PROBE_EXPR_EVALUATORS,
        this_ptr, "evaluator_vector");
    ...
    for (int i = 0; i < exprs.size(); ++i) {
      ...
      // Load evaluators[i] and call GetValue()
      Value* evaluator_arg =
          codegen->CodegenArrayAt(&builder, evaluator_vector, i, "evaluator");
      DCHECK(evaluator_arg->getType()->isPointerTy());
      CodegenAnyVal result = CodegenAnyVal::CreateCallWrapped(codegen, &builder,
          exprs[i]->type(), expr_fn, {evaluator_arg, row}, "result");
...
bool OldHashTable::Equals(TupleRow* build_row) {
  for (int i = 0; i < build_exprs_.size(); ++i) {
    void* val = build_expr_evaluators_[i]->GetValue(build_row);
    if (val == NULL) {
...
Function* OldHashTable::CodegenEquals(LlvmCodeGen* codegen) {
  ...
  if (!build_exprs_.empty()) {
    ...
    // Load &build_expr_evaluators_[0]
    Value* evaluator_vector = codegen->CodegenCallFunction(&builder,
        IRFunction::OLD_HASH_TABLE_GET_BUILD_EXPR_EVALUATORS,
        this_ptr, "evaluator_vector");
    ...
    for (int i = 0; i < build_exprs_.size(); ++i) {
      ...
      // Generate GetValue() of build_expr_evaluators_[i]
      Function* expr_fn;
      ...
      // Call GetValue() on build_expr_evaluators_[i]
      Value* evaluator_arg =
          codegen->CodegenArrayAt(&builder, evaluator_vector, i, "evaluator");
      CodegenAnyVal result = CodegenAnyVal::CreateCallWrapped(codegen, &builder,
          build_exprs_[i]->type(), expr_fn, {evaluator_arg, row}, "result");
      Value* is_null = result.GetIsNull();
