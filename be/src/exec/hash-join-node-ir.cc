...
int HashJoinNode::ProcessProbeBatch(RowBatch* out_batch, RowBatch* probe_batch,
  ...
  ScalarExprEvaluator* const* other_conjunct_evaluators =
      other_join_conjunct_evaluators_.data();
  ...
  ScalarExprEvaluator* const* conjunct_evaluators = conjunct_evaluators_.data();
    ...
    while (!hash_tbl_iterator_.AtEnd()) {
      ...
      if (join_op_ == TJoinOp::LEFT_SEMI_JOIN) {
        ...
        if (num_other_conjuncts > 0) {
          ...
          if (!EvalOtherJoinConjuncts2(other_conjunct_evaluators, num_other_conjuncts,
        ...
        if (!EvalOtherJoinConjuncts2(
                other_conjunct_evaluators, num_other_conjuncts, out_row)) {
          continue;
        }
      }
      matched_probe_ = true;

      if (EvalConjuncts(conjunct_evaluators, num_conjuncts, out_row)) {
        ++rows_returned;
