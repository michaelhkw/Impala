...
void IR_ALWAYS_INLINE UnionNode::MaterializeExprs(
    const vector<ScalarExprEvaluator*>& evaluators, TupleRow* row, uint8_t* tuple_buf,
    RowBatch* dst_batch) {
  ...
  dst_tuple->MaterializeExprs<false, false>(row, *tuple_desc_, evaluators,
...
void UnionNode::MaterializeBatch(RowBatch* dst_batch, uint8_t** tuple_buf) {
  ...
  const std::vector<ScalarExprEvaluator*>& child_expr_evaluators =
      child_expr_evaluators_lists_[child_idx_];
  ...
  FOREACH_ROW_LIMIT(child_batch, child_row_idx_, num_rows_to_process, batch_iter) {
    ...
    MaterializeExprs(child_expr_evaluators, child_row, cur_tuple, dst_batch);
