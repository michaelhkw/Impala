...
PartitionedAggregationNode::PartitionedAggregationNode(
  ...
  : ExecNode(pool, tnode, descs),
    ...
    intermediate_row_desc_(new RowDescriptor(intermediate_tuple_desc_, false)),
  ...
  if (is_streaming_preagg_) {
    DCHECK(conjunct_evaluators_.empty()) << "Preaggs have no conjuncts";
...
Status PartitionedAggregationNode::Init(const TPlanNode& tnode, RuntimeState* state) {
  ...
  RETURN_IF_ERROR(ScalarExpr::Create(pool_, state, row_desc,
      tnode.agg_node.grouping_exprs, &grouping_exprs_));
  ...
  for (int i = 0; i < grouping_exprs_.size(); ++i) {
    ...
    ScalarExpr* build_expr = desc->type().type != TYPE_NULL ?
        new SlotRef(desc) : new SlotRef(desc, TYPE_BOOLEAN);
    pool_->Add(build_expr);
    build_exprs_.push_back(build_expr);
    RETURN_IF_ERROR(build_expr->Init(state, *(intermediate_row_desc_.get())));
  ...
  for (int i = 0; i < tnode.agg_node.aggregate_functions.size(); ++i, ++j) {
    ...
    AggFn* agg_fn;
    RETURN_IF_ERROR(AggFn::Create(pool_, state, row_desc, intermediate_slot_desc,
        output_slot_desc, tnode.agg_node.aggregate_functions[i], &agg_fn));
...
Status PartitionedAggregationNode::Prepare(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(AggFnEvaluator::Create(pool_, state, agg_fn_pool_.get(),
      agg_fns_, &agg_fn_evaluators_));
...
Status PartitionedAggregationNode::Open(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(AggFnEvaluator::Open(agg_fn_evaluators_, state));
  if (grouping_exprs_.empty()) {
    ...
    singleton_output_tuple_ =
        ConstructSingletonOutputTuple(agg_fn_evaluators_, mem_pool_.get());
...
void PartitionedAggregationNode::GetSingletonOutput(RowBatch* row_batch) {
  ...
  Tuple* output_tuple = GetOutputTuple(agg_fn_evaluators_,
      singleton_output_tuple_, row_batch->tuple_data_pool());
  row->SetTuple(0, output_tuple);
  if (ExecNode::EvalConjuncts(&conjunct_evaluators_[0], conjunct_evaluators_.size(), row)) {
...
Status PartitionedAggregationNode::GetRowsFromPartition(RuntimeState* state,
  ...
  while (!output_iterator_.AtEnd()) {
    ...
    Tuple* output_tuple = GetOutputTuple(output_partition_->agg_fn_evaluators,
        intermediate_tuple, row_batch->tuple_data_pool());
    output_iterator_.Next();
    row->SetTuple(0, output_tuple);
    DCHECK_EQ(conjunct_evaluators_.size(), conjuncts_.size());
    if (ExecNode::EvalConjuncts(&conjunct_evaluators_[0], conjuncts_.size(), row)) {
      row_batch->CommitLastRow();
      ++num_rows_returned_;
...
void PartitionedAggregationNode::CleanupHashTbl(
    vector<AggFnEvaluator*>& agg_fn_evaluators, HashTable::Iterator it) {
  ...
  if (needs_finalize_) {
    ...
    while (!it.AtEnd()) {
      ...
      AggFnEvaluator::Finalize(agg_fn_evaluators, tuple, dummy_dst);
      it.Next();
    }
  } else {
    while (!it.AtEnd()) {
      Tuple* tuple = it.GetTuple();
      AggFnEvaluator::Serialize(agg_fn_evaluators, tuple);
      it.Next();
    }
  }
}
...
void PartitionedAggregationNode::Close(RuntimeState* state) {
  ...
  if (!singleton_output_tuple_returned_) {
    GetOutputTuple(agg_fn_evaluators_, singleton_output_tuple_, mem_pool_.get());
  }

  // Iterate through the remaining rows in the hash table and call Serialize/Finalize on
  // them in order to free any memory allocated by UDAs
  if (output_partition_ != NULL) {
    CleanupHashTbl(output_partition_->agg_fn_evaluators, output_iterator_);
    output_partition_->Close(false);
  }
  ...
  AggFnEvaluator::Close(agg_fn_evaluators_, state);
...
Status PartitionedAggregationNode::Partition::InitStreams() {
  ...
  DCHECK_EQ(agg_fn_evaluators.size(), 0);
  AggFnEvaluator::Clone(parent->pool_, agg_fn_pool.get(), parent->agg_fn_evaluators_,
      &agg_fn_evaluators);
...
Status PartitionedAggregationNode::Partition::SerializeStreamForSpilling() {
  ...
  if (parent->needs_serialize_ && aggregated_row_stream->num_rows() != 0) {
    ...
    // where we are).
    if (failed_to_add) {
      parent->CleanupHashTbl(agg_fn_evaluators, it);
      hash_tbl->Close();
      hash_tbl.reset();
...
Status PartitionedAggregationNode::Partition::Spill() {
  ...
  AggFnEvaluator::Close(agg_fn_evaluators, parent->state_);
  agg_fn_evaluators.clear();
...
void PartitionedAggregationNode::Partition::Close(bool finalize_rows) {
  ...
  if (aggregated_row_stream.get() != NULL) {
    if (finalize_rows && hash_tbl.get() != NULL) {
      ...
      parent->CleanupHashTbl(agg_fn_evaluators, hash_tbl->Begin(parent->ht_ctx_.get()));
    }
    aggregated_row_stream->Close(NULL, RowBatch::FlushMode::NO_FLUSH_RESOURCES);
  }
  ...
  for (AggFnEvaluator* evaluator : agg_fn_evaluators) evaluator->Close(parent->state_);
  if (agg_fn_pool.get() != NULL) agg_fn_pool->FreeAll();
}

Tuple* PartitionedAggregationNode::ConstructSingletonOutputTuple(
    vector<AggFnEvaluator*>& agg_fn_evaluators, MemPool* pool) {
  DCHECK(grouping_exprs_.empty());
  Tuple* output_tuple = Tuple::Create(intermediate_tuple_desc_->byte_size(), pool);
  InitAggSlots(agg_fn_evaluators, output_tuple);
  return output_tuple;
}

Tuple* PartitionedAggregationNode::ConstructIntermediateTuple(
    vector<AggFnEvaluator*>& agg_fn_evaluators, MemPool* pool, Status* status) noexcept {
  ...
  InitAggSlots(agg_fn_evaluators, intermediate_tuple);
  return intermediate_tuple;
}

Tuple* PartitionedAggregationNode::ConstructIntermediateTuple(
    vector<AggFnEvaluator*>& agg_fn_evaluators, BufferedTupleStream* stream,
...
void PartitionedAggregationNode::InitAggSlots(
    vector<AggFnEvaluator*>& agg_fn_evaluators, Tuple* intermediate_tuple) {
  vector<SlotDescriptor*>::const_iterator slot_desc =
      intermediate_tuple_desc_->slots().begin() + grouping_exprs_.size();
  for (int i = 0; i < agg_fn_evaluators.size(); ++i, ++slot_desc) {
    ...
    AggFnEvaluator* evaluator = agg_fn_evaluators[i];
    evaluator->Init(intermediate_tuple);

    DCHECK(agg_fns_[i] == evaluator->agg_fn());
...
void PartitionedAggregationNode::UpdateTuple(AggFnEvaluator** agg_fn_evaluators,
    Tuple* tuple, TupleRow* row, bool is_merge) noexcept {
  DCHECK(tuple != NULL || agg_fns_.empty());
  for (int i = 0; i < agg_fns_.size(); ++i) {
    if (is_merge) {
      agg_fn_evaluators[i]->Merge(row->GetTuple(0), tuple);
    } else {
      agg_fn_evaluators[i]->Add(row, tuple);
    }
  }
}

Tuple* PartitionedAggregationNode::GetOutputTuple(
    vector<AggFnEvaluator*>& agg_fn_evaluators, Tuple* tuple, MemPool* pool) {
  DCHECK(tuple != NULL || agg_fn_evaluators.empty()) << tuple;
  Tuple* dst = tuple;
  if (needs_finalize_ && intermediate_tuple_id_ != output_tuple_id_) {
    dst = Tuple::Create(output_tuple_desc_->byte_size(), pool);
  }
  if (needs_finalize_) {
    AggFnEvaluator::Finalize(agg_fn_evaluators, tuple, dst);
  } else {
    AggFnEvaluator::Serialize(agg_fn_evaluators, tuple);
  }
...
Status PartitionedAggregationNode::QueryMaintenance(RuntimeState* state) {
  AggFnEvaluator::FreeLocalAllocations(agg_fn_evaluators_);
  for (int i = 0; i < hash_partitions_.size(); ++i) {
    AggFnEvaluator::FreeLocalAllocations(hash_partitions_[i]->agg_fn_evaluators);
  }
...
// void UpdateSlot(FunctionContext* agg_fn_ctx, ScalarExprEvaluator* agg_expr_evaluator,
...
//    %"class.impala::ScalarExprEvaluator"** %agg_expr_evaluators,
//    { i8, [7 x i8], double }* %agg_tuple, %"class.impala::TupleRow"* %row) #34 {
// entry:
//   %expr_evaluator_ptr = getelementptr %"class.impala::ScalarExprEvaluator"*,
//      %"class.impala::ScalarExprEvaluator"** %agg_expr_evaluators, i32 0
//   %expr_evaluator = load %"class.impala::ScalarExprEvaluator"*,
//      %"class.impala::ScalarExprEvaluator"** %expr_evaluator_ptr
//   %input0 = call { i8, double } @GetSlotRef(%"class.impala::ScalarExprEvaluator"* %expr_evaluator,
...
//   %expr_evaluator = load %"class.impala::ScalarExprEvaluator"*,
//        %"class.impala::ScalarExprEvaluator"** %expr_evaluator_ptr
//   %input0 = call { i64, i64 } @GetSlotRef(%"class.impala::ScalarExprEvaluator"* %expr_evaluator,
//        %"class.impala::TupleRow"* %row)
...
Status PartitionedAggregationNode::CodegenUpdateSlot(LlvmCodeGen* codegen,
  ...
  PointerType* agg_fn_evaluator_type =
  ...
  prototype.AddArgument(
      LlvmCodeGen::NamedVariable("agg_fn_evaluator", agg_fn_evaluator_type));
  ...
  *fn = prototype.GeneratePrototype(&builder, &args[0]);
  Value* agg_fn_evaluator_arg = args[0];
  Value* agg_tuple_arg = args[1];
  Value* row_arg = args[2];

  // Get the vector of input expressions' evaluators.
  Value* input_evaluators_vector = codegen->CodegenCallFunction(&builder,
      IRFunction::AGG_FN_EVALUATOR_INPUT_EVALUATORS, agg_fn_evaluator_arg,
      "input_evaluators_vector");
  ...
  for (int i = 0; i < num_inputs; ++i) {
    ...
    Value* input_evaluator =
        codegen->CodegenArrayAt(&builder, input_evaluators_vector, i, "input_evaluator");
    string input_name = Substitute("input$0", i);
    CodegenAnyVal input_val = CodegenAnyVal::CreateCallWrapped(codegen, &builder,
        input_expr->type(), input_expr_fn, ArrayRef<Value*>({input_evaluator, row_arg}),
        input_name.c_str());
  ...
  if (agg_op == AggFn::COUNT) {
    ...
    Value* agg_fn_ctx_val =
        builder.CreateCall(get_agg_fn_ctx_fn, {agg_fn_evaluator_arg}, "agg_fn_ctx");
...
//      @_ZNK6impala26PartitionedAggregationNode18GetAggScalarExprEvaluatorsEi(
//      %"class.impala::PartitionedAggregationNode"* %this_ptr, i32 1)
//   call void @UpdateSlot(%"class.impala_udf::FunctionContext"* %agg_fn_ctx,
//      %"class.impala::ScalarExprEvaluator"** %1, { i8, [7 x i8], i64, i64, double }* %tuple1,
//      %"class.impala::TupleRow"* %row)
...
//   %3 = call %"class.impala::ScalarExprEvaluator"**
//      @_ZNK6impala26PartitionedAggregationNode18GetAggScalarExprEvaluatorsEi(
...
//      %"class.impala::ScalarExprEvaluator"** %3, { i8, [7 x i8], i64, i64, double }* %tuple1,
//      %"class.impala::TupleRow"* %row)
//   ret void
// }
Status PartitionedAggregationNode::CodegenUpdateTuple(
  ...
  PointerType* evaluators_type = codegen->GetPtrPtrType(AggFnEvaluator::LLVM_CLASS_NAME);
  ...
  prototype.AddArgument(LlvmCodeGen::NamedVariable("agg_fn_evaluators", evaluators_type));
  ...
  Value* args[5];
  *fn = prototype.GeneratePrototype(&builder, &args[0]);
  Value* agg_fn_evaluators_arg = args[1];
  ...
  for (int i = 0; i < agg_fns_.size(); ++i, ++j) {
    ...
    if (agg_fn->is_count_star()) {
      ...
      // Load agg_fn_evaluators_[i]
      Value* agg_fn_evaluator_val =
          codegen->CodegenArrayAt(&builder, agg_fn_evaluators_arg, i, "agg_fn_evaluator");

      // Call UpdateSlot(agg_fn_evaluators_[i], tuple, row);
      Value* update_slot_args[] = {agg_fn_evaluator_val, tuple_arg, row_arg};
