...
Status KuduTableSink::Send(RuntimeState* state, RowBatch* batch) {
  ...
  for (int i = 0; i < batch->num_rows(); ++i) {
    ...
    for (int j = 0; j < output_expr_evaluators_.size(); ++j) {
      // output_expr_evaluators_ only contains the columns that the op
      ...
      PrimitiveType type = output_expr_evaluators_[j]->root().type().type;
