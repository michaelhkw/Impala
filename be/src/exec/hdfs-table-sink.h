...
class HdfsTableSink : public DataSink {
 ...
 protected:
  ...
  virtual Status Init(RuntimeState* state, const TDataSink& tsink,
      const std::vector<TExpr>& thrift_output_exprs) WARN_UNUSED_RESULT;
 ...
 private:
  ...
  void GetHashTblKey(const TupleRow* row,
      const std::vector<ScalarExprEvaluator*>& evaluators, std::string* key);
  ...
  std::vector<ScalarExprEvaluator*> partition_key_expr_evaluators_;

  /// Subset of partition_key_expr_evaluators_ which are not constant. Set in Prepare().
  /// Used for generating the string key of hash_tbl_.
  std::vector<ScalarExprEvaluator*> dynamic_partition_key_expr_evaluators_;
