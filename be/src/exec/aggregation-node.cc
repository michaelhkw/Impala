...
Status AggregationNode::Init(const TPlanNode& tnode, RuntimeState* state) {
  ...
  for (int i = 0; i < grouping_exprs_.size(); ++i) {
    ...
    RETURN_IF_ERROR(build_expr->Init(state, build_row_desc));
...
Status AggregationNode::Prepare(RuntimeState* state) {
  ...
  DCHECK_EQ(agg_fns_.size(), agg_fn_evaluators_.size());
...
Status AggregationNode::GetNext(RuntimeState* state, RowBatch* row_batch, bool* eos) {
  ...
  *eos = false;
  ScalarExprEvaluator* const* evaluators = &conjunct_evaluators_[0];
  int num_conjuncts = conjuncts_.size();
  DCHECK_EQ(num_conjuncts, conjunct_evaluators_.size());
  ...
  while (!output_iterator_.AtEnd() && !row_batch->AtCapacity()) {
    ...
    if (ExecNode::EvalConjuncts(evaluators, num_conjuncts, row)) {
...
void AggregationNode::Close(RuntimeState* state) {
  ...
  while (!output_iterator_.AtEnd()) {
    ...
    if (needs_finalize_) {
      AggFnEvaluator::Finalize(agg_fn_evaluators_, tuple, dummy_dst);
    } else {
      AggFnEvaluator::Serialize(agg_fn_evaluators_, tuple);
    }
    output_iterator_.Next<false>();
  }
...
Tuple* AggregationNode::ConstructIntermediateTuple() {
  ...
  for (int i = 0; i < agg_fns_.size(); ++i, ++slot_desc) {
    AggFnEvaluator* evaluator = agg_fn_evaluators_[i];
...
void AggregationNode::UpdateTuple(Tuple* tuple, TupleRow* row) {
  DCHECK(tuple != nullptr || agg_fn_evaluators_.empty());
  AggFnEvaluator::Add(agg_fn_evaluators_, row, tuple);
}
...
Tuple* AggregationNode::FinalizeTuple(Tuple* tuple, MemPool* pool) {
  ...
  if (needs_finalize_) {
    AggFnEvaluator::Finalize(agg_fn_evaluators_, tuple, dst);
  } else {
    AggFnEvaluator::Serialize(agg_fn_evaluators_, tuple);
  }
...
// void UpdateSlot(FunctionContext* fn_ctx, ScalarExprEvaluator* expr_evaluator,
//     AggTuple* agg_tuple, char** row)
//
// The IR for sum(double_col) is:
// define void @UpdateSlot(%"class.impala_udf::FunctionContext"* %fn_ctx,
//                         %"class.impala::ScalarExprEvaluator"* %expr_evaluator,
//                         { i8, [7 x i8], double }* %agg_tuple,
//                         %"class.impala::TupleRow"* %row) #34 {
// entry:
//   %src = call { i8, double } @GetSlotRef(%"class.impala::ScalarExprEvaluator"* %expr_evaluator,
...
//   %src = call { i64, i8* } @GetSlotRef(%"class.impala::ScalarExprEvaluator"* %expr_evaluator,
//                                        %"class.impala::TupleRow"* %row)
...
llvm::Function* AggregationNode::CodegenUpdateSlot(LlvmCodeGen* codegen,
  ...
  Value* input_evaluator =
      codegen->CodegenArrayAt(&builder, input_evaluators_vector, 0, "input_evaluator");

  // Call expr function to get src slot value
  CodegenAnyVal src = CodegenAnyVal::CreateCallWrapped(codegen, &builder,
      input_expr->type(), input_expr_fn, {input_evaluator, row_arg}, "src");
  ...
  switch (agg_fn->agg_op()) {
    ...
    case AggFn::NDV: {
      ...
      Value* agg_fn_ctx_arg = codegen->CodegenCallFunction(&builder,
          IRFunction::AGG_FN_EVALUATOR_AGG_FN_CTX, agg_fn_evaluator_arg,
          "agg_fn_ctx_arg");
...
Function* AggregationNode::CodegenUpdateTuple(LlvmCodeGen* codegen) {
  ...
  //     ScalarExprEvaluator** expr_evaluator, Tuple* tuple, TupleRow* row)
  ...
  // Load &agg_fn_evaluators_[0]
  Value* agg_fn_evaluators_vector = codegen->CodegenCallFunction(&builder,
      IRFunction::AGG_NODE_GET_AGG_FN_EVALUATORS, this_arg, "agg_fn_evaluators");
  ...
  for (int i = 0; i < agg_fns_.size(); ++i, ++j) {
    ...
    if (agg_fn->is_count_star()) {
      ...
      DCHECK(agg_fn_evaluators_[i] != nullptr);
      Value* agg_fn_evaluator_arg = codegen->CodegenArrayAt(
          &builder, agg_fn_evaluators_vector, i, "agg_fn_evaluator");
      builder.CreateCall(update_slot_fn, {agg_fn_evaluator_arg, agg_tuple_arg, row_arg});
    }
  }
  builder.CreateRetVoid();
