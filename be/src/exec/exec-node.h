...
class ExecNode {
 public:
  ...
  /// Evaluates the conjunct in 'evaluator' over 'row' and returns the result.
  static bool EvalConjunct(ScalarExprEvaluator* evaluator, TupleRow* row);
  ...
  const std::vector<ScalarExprEvaluator*>& conjunct_evaluators() const {
    return conjunct_evaluators_;
  }
 ...
 protected:
  ...
  /// Conjuncts and their evaluators in this node.
  ...
  /// Frees any local allocations made by evaluators_to_free_ and returns the result of
  ...
  void AddEvaluatorToFree(ScalarExprEvaluator* evaluator);
  void AddEvaluatorsToFree(const std::vector<ScalarExprEvaluator*>& evaluators);

  /// Free any local allocations made by evaluators_to_free_.
 ...
 private:
  ...
  /// thread.
  std::vector<ScalarExprEvaluator*> evaluators_to_free_;
};
