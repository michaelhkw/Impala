...
Status HashTableCtx::Init(ObjectPool* pool, RuntimeState* state, int num_build_tuples) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(pool, state, mem_pool_,
...
bool HashTableCtx::Equals(const TupleRow* build_row, const uint8_t* expr_values,
  ...
  for (int i = 0; i < build_expr_evaluators_.size(); ++i) {
    ...
    const void* loc = expr_values_cache_.ExprValuePtr(expr_values, i);
    DCHECK(build_exprs_[i] == build_expr_evaluators_[i]->root());
...
// Codegen for evaluating a tuple row over either build_expr_evaluators_ or
// probe_expr_evaluators_. For a group by with (big int, string) the IR looks like:
//
...
Status HashTableCtx::CodegenEvalRow(LlvmCodeGen* codegen, bool build, Function** fn) {
  ...
  *fn = prototype.GeneratePrototype(&builder, args);
  ...
  // evaluator_vector = &build_expr_evaluators_[0] / &probe_expr_evaluators_[0]
  ...
  for (int i = 0; i < exprs.size(); ++i) {
    ...
    CodegenAnyVal result = CodegenAnyVal::CreateCallWrapped(
        codegen, &builder, exprs[i]->type(), expr_fn, {evaluator_arg, row}, "result");
...
Status HashTableCtx::CodegenEquals(LlvmCodeGen* codegen, bool force_null_equality,
  ...
  *fn = prototype.GeneratePrototype(&builder, args);
  ...
  // evaluator_vector = &build_expr_evaluators_[0]
  Value* evaluator_vector = codegen->CodegenCallFunction(&builder,
      IRFunction::HASH_TABLE_GET_BUILD_EXPR_EVALUATORS, this_ptr, "evaluator_vector");
  ...
  for (int i = 0; i < build_exprs_.size(); ++i) {
    ...
    // Load ScalarExprEvaluator*: evaluator = evaluator_vector[i];
    Value* evaluator_arg =
        codegen->CodegenArrayAt(&builder, evaluator_vector, i, "evaluator");
