...
bool IR_NO_INLINE EvalOtherJoinConjuncts(
    ScalarExprEvaluator* const* evaluators, int num_evaluators, TupleRow* row) {
  return ExecNode::EvalConjuncts(evaluators, num_evaluators, row);
}

bool IR_ALWAYS_INLINE PartitionedHashJoinNode::ProcessProbeRowInnerJoin(
    ScalarExprEvaluator* const* other_join_conjunct_evaluators,
    int num_other_join_conjuncts, ScalarExprEvaluator* const* conjunct_evaluators,
  ...
  for (; !hash_tbl_iterator_.AtEnd(); hash_tbl_iterator_.NextDuplicate()) {
    ...
    if (!EvalOtherJoinConjuncts(other_join_conjunct_evaluators, num_other_join_conjuncts,
        out_row)) {
      continue;
    }
    if (ExecNode::EvalConjuncts(conjunct_evaluators, num_conjuncts, out_row)) {
      --(*remaining_capacity);
...
bool IR_ALWAYS_INLINE PartitionedHashJoinNode::ProcessProbeRowRightSemiJoins(
  ...
  for (; !hash_tbl_iterator_.AtEnd(); hash_tbl_iterator_.NextDuplicate()) {
    ...
    if (num_other_join_conjuncts > 0) {
      ...
      if (!EvalOtherJoinConjuncts(other_join_conjunct_evaluators,
    ...
    if (JoinOp == TJoinOp::RIGHT_SEMI_JOIN &&
        ExecNode::EvalConjuncts(conjunct_evaluators, num_conjuncts, out_row)) {
      --(*remaining_capacity);
      if (*remaining_capacity == 0) {
        hash_tbl_iterator_.NextDuplicate();
        return false;
...
bool IR_ALWAYS_INLINE PartitionedHashJoinNode::ProcessProbeRowLeftSemiJoins(
  ...
  if (JoinOp != TJoinOp::LEFT_SEMI_JOIN && !matched_probe_) {
    ...
    // evaluate the conjunct_evaluators since anti joins cannot have any.
    out_batch_iterator->parent()->CopyRow(current_probe_row_, out_row);
    matched_probe_ = true;
    --(*remaining_capacity);
    if (*remaining_capacity == 0) return false;
...
bool IR_ALWAYS_INLINE PartitionedHashJoinNode::ProcessProbeRow(
  ...
  if (JoinOp == TJoinOp::INNER_JOIN) {
    return ProcessProbeRowInnerJoin(other_join_conjunct_evaluators,
        num_other_join_conjuncts, conjunct_evaluators, num_conjuncts, out_batch_iterator,
        remaining_capacity);
  } else if (JoinOp == TJoinOp::RIGHT_SEMI_JOIN ||
             JoinOp == TJoinOp::RIGHT_ANTI_JOIN) {
    return ProcessProbeRowRightSemiJoins<JoinOp>(other_join_conjunct_evaluators,
        num_other_join_conjuncts, conjunct_evaluators, num_conjuncts, out_batch_iterator,
        remaining_capacity);
  } else if (JoinOp == TJoinOp::LEFT_SEMI_JOIN ||
             JoinOp == TJoinOp::LEFT_ANTI_JOIN ||
             JoinOp == TJoinOp::NULL_AWARE_LEFT_ANTI_JOIN) {
    return ProcessProbeRowLeftSemiJoins<JoinOp>(other_join_conjunct_evaluators,
        num_other_join_conjuncts, conjunct_evaluators, num_conjuncts, out_batch_iterator,
        remaining_capacity, status);
  } else {
    DCHECK(JoinOp == TJoinOp::RIGHT_OUTER_JOIN ||
           JoinOp == TJoinOp::LEFT_OUTER_JOIN || TJoinOp::FULL_OUTER_JOIN);
    return ProcessProbeRowOuterJoins<JoinOp>(other_join_conjunct_evaluators,
        num_other_join_conjuncts, conjunct_evaluators, num_conjuncts, out_batch_iterator,
        remaining_capacity);
  }
}
  ...
  while (!expr_vals_cache->AtEnd()) {
    ...
    if (expr_vals_cache->IsRowNull()) {
      if (JoinOp == TJoinOp::NULL_AWARE_LEFT_ANTI_JOIN && builder_->non_empty_build()) {
        const int num_other_join_conjuncts = other_join_conjunct_evaluators_.size();
...
int PartitionedHashJoinNode::ProcessProbeBatch(TPrefetchMode::type prefetch_mode,
  ...
  ScalarExprEvaluator* const* other_join_conjunct_evaluators =
      &other_join_conjunct_evaluators_[0];
  const int num_other_join_conjuncts = other_join_conjunct_evaluators_.size();
  ScalarExprEvaluator* const* conjunct_evaluators = &conjunct_evaluators_[0];
  const int num_conjuncts = conjunct_evaluators_.size();
  ...
  while (has_probe_rows && remaining_capacity > 0 && status->ok()) {
    ...
    do {
      ...
      if (current_probe_row_ != NULL) {
        if (!ProcessProbeRow<JoinOp>(other_join_conjunct_evaluators,
                num_other_join_conjuncts, conjunct_evaluators, num_conjuncts,
