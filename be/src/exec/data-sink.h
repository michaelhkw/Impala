...
class DataSink {
 public:
  ...
  /// Initializes the output expression evaluators. Also creates a MemPool and MemTracker
  /// for expressions for the sink that is a child of 'parent_mem_tracker'. Called during
  /// the prepare phase of the query. Subclasses must call DataSink::Prepare().
  ...
  static Status CreateDataSink(RuntimeState* state,
