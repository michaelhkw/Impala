...
class HashTableCtx {
 ...
 private:
  ...
  bool IR_NO_INLINE EvalBuildRow(
    ...
    return EvalRow(row, build_expr_evaluators_, expr_values, expr_values_null);
  }
  ...
  bool IR_NO_INLINE EvalProbeRow(
      const TupleRow* row, uint8_t* expr_values, uint8_t* expr_values_null) noexcept {
    return EvalRow(row, probe_expr_evaluators_, expr_values, expr_values_null);
  }
  ...
  ScalarExprEvaluator* const* IR_ALWAYS_INLINE GetBuildExprEvaluators() const;
  ScalarExprEvaluator* const* IR_ALWAYS_INLINE GetProbeExprEvaluators() const;
  ...
  std::vector<ScalarExprEvaluator*> build_expr_evaluators_;

  /// The exprs used to evaluate rows for look-up in the hash table.
  const std::vector<ScalarExpr*>& probe_exprs_;
  std::vector<ScalarExprEvaluator*> probe_expr_evaluators_;
  ...
  /// MemPool for 'build_expr_evaluators_' and 'probe_expr_evaluators_' to allocate from.
  /// Not owned.
  MemPool* mem_pool_;
};
