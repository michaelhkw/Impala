...
Status UnnestNode::Prepare(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(pool_, state, expr_mem_pool(),
      coll_expr_, &coll_expr_evaluator_));
...
Status UnnestNode::Open(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(coll_expr_evaluator_->Open(state));
...
Status UnnestNode::GetNext(RuntimeState* state, RowBatch* row_batch, bool* eos) {
  ...
  *eos = false;
  ...
  while (item_idx_ < coll_value_->num_tuples) {
    ...
    DCHECK_EQ(conjuncts_.size(), conjunct_evaluators_.size());
    if (EvalConjuncts(&conjunct_evaluators_[0], conjuncts_.size(), row)) {
...
void UnnestNode::Close(RuntimeState* state) {
  if (is_closed()) return;
  if (coll_expr_evaluator_ != nullptr) coll_expr_evaluator_->Close(state);
