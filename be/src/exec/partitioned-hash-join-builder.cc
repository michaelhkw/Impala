...
Status PhjBuilder::Init(RuntimeState* state, const TDataSink& tsink,
...
Status PhjBuilder::Init(RuntimeState* state,
  ...
  for (const TEqJoinCondition& eq_join_conjunct : eq_join_conjuncts) {
    ScalarExpr* build_expr = nullptr;
    RETURN_IF_ERROR(ScalarExpr::Create(&pool_, state, row_desc_,
  ...
  for (const TRuntimeFilterDesc& filter : filters) {
    ...
    // TODO: filter_expr should be owned by the filter itself, not the filter context.
    RETURN_IF_ERROR(
        ScalarExpr::Create(&pool_, state, row_desc_, filter.src_expr, &filter_ctx.expr));
...
void PhjBuilder::FreeLocalAllocations() const {
  ...
  for (const FilterContext& ctx : filter_ctxs_) {
    if (ctx.expr_evaluator != nullptr) ctx.expr_evaluator->FreeLocalAllocations();
  }
}
...
void PhjBuilder::Close(RuntimeState* state) {
  ...
  ht_ctx_.reset();
  for (const FilterContext& ctx : filter_ctxs_) {
    if (ctx.expr_evaluator != nullptr) ctx.expr_evaluator->Close(state);
    if (ctx.expr != nullptr) ctx.expr->Close();
  }
