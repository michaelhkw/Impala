...
HdfsTextTableWriter::HdfsTextTableWriter(HdfsTableSink* parent,
    ...
    : HdfsTableWriter(parent, state, output, partition, table_desc, evaluators) {
...
Status HdfsTextTableWriter::AppendRows(
  ...
  DCHECK_GE(output_expr_evaluators_.size(), num_non_partition_cols) << parent_->DebugString();

  {
    ...
    for (int row_idx = 0; row_idx < limit; ++row_idx) {
      ...
      for (int j = 0; j < num_non_partition_cols; ++j) {
        void* value = output_expr_evaluators_[j]->GetValue(current_row);
        if (value != NULL) {
          const ColumnType& type = output_expr_evaluators_[j]->root()->type();
          if (type.type == TYPE_CHAR) {
            ...
            output_expr_evaluators_[j]->PrintValue(value, &rowbatch_stringstream_);
          }
        } else {
