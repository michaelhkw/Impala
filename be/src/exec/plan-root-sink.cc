...
void PlanRootSink::GetRowValue(
  ...
  DCHECK(result->size() >= output_expr_evaluators_.size());
  for (int i = 0; i < output_expr_evaluators_.size(); ++i) {
    (*result)[i] = output_expr_evaluators_[i]->GetValue(row);
    (*scales)[i] = output_expr_evaluators_[i]->output_scale();
  }
}
}
