...
Status NestedLoopJoinNode::Init(const TPlanNode& tnode, RuntimeState* state) {
  ...
  // join_conjunct_evaluators_ are evaluated in the context of rows assembled from
  // all inner and outer tuples.
  RowDescriptor full_row_desc(child(0)->row_desc(), child(1)->row_desc());
  RETURN_IF_ERROR(ScalarExpr::Create(pool_, state, full_row_desc,
      tnode.nested_loop_join_node.join_conjuncts, &join_conjuncts_));
...
Status NestedLoopJoinNode::Open(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Open(join_conjunct_evaluators_, state));
...
Status NestedLoopJoinNode::Prepare(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(pool_, state, expr_mem_pool(),
      join_conjuncts_, &join_conjunct_evaluators_));
...
void NestedLoopJoinNode::Close(RuntimeState* state) {
  if (is_closed()) return;
  ScalarExprEvaluator::Close(join_conjunct_evaluators_, state);
...
Status NestedLoopJoinNode::GetNextLeftSemiJoin(RuntimeState* state,
    RowBatch* output_batch) {
  ScalarExprEvaluator* const* join_conjunct_evaluators = &join_conjunct_evaluators_[0];
  size_t num_join_conjuncts = join_conjuncts_.size();
  DCHECK_EQ(num_join_conjuncts, join_conjunct_evaluators_.size());
  ...
  while (!eos_) {
    ...
    while (!build_row_iterator_.AtEnd()) {
      ...
      if (!EvalConjuncts(
              join_conjunct_evaluators, num_join_conjuncts, semi_join_staging_row_)) {
        continue;
      }
...
Status NestedLoopJoinNode::ProcessUnmatchedProbeRow(RuntimeState* state,
  ...
  if (EvalConjuncts(conjunct_evaluators, num_conjuncts, output_row)) {
    VLOG_ROW << "match row:" << PrintRow(output_row, row_desc());
    output_batch->CommitLastRow();
    ++num_rows_returned_;
...
Status NestedLoopJoinNode::FindBuildMatches(
  ...
  *return_output_batch = false;
  ...
  while (!build_row_iterator_.AtEnd()) {
    ...
    if (!EvalConjuncts(join_conjunct_evaluators, num_join_conjuncts, output_row)) {
      continue;
    }
    matched_probe_ = true;
    if (matching_build_rows_ != NULL) {
    ...
    if (!EvalConjuncts(conjunct_evaluators, num_conjuncts, output_row)) continue;
    VLOG_ROW << "match row: " << PrintRow(output_row, row_desc());
    output_batch->CommitLastRow();
    ++num_rows_returned_;
    if (output_batch->AtCapacity()) {
