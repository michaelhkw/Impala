...
Status PartitionedHashJoinNode::Init(const TPlanNode& tnode, RuntimeState* state) {
  ...
  for (const TEqJoinCondition& eq_join_conjunct : eq_join_conjuncts) {
    ...
    RETURN_IF_ERROR(ScalarExpr::Create(pool_, state, child(0)->row_desc(),
        eq_join_conjunct.left, &probe_expr));
    probe_exprs_.push_back(probe_expr);
    ScalarExpr* build_expr;
    RETURN_IF_ERROR(ScalarExpr::Create(pool_, state, child(1)->row_desc(),
        eq_join_conjunct.right, &build_expr));
    build_exprs_.push_back(build_expr);
  }
  // other_join_conjuncts_ are evaluated in the context of rows assembled from all build
  // nd probe tuples; full_row_desc is not necessarily the same as the output row desc,
  ...
  RETURN_IF_ERROR(ScalarExpr::Create(pool_, state, full_row_desc,
      tnode.hash_join_node.other_join_conjuncts, &other_join_conjuncts_));
...
Status PartitionedHashJoinNode::Prepare(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(pool_, state, expr_mem_pool(),
      other_join_conjuncts_, &other_join_conjunct_evaluators_));
  AddEvaluatorsToFree(other_join_conjunct_evaluators_);
...
Status PartitionedHashJoinNode::Open(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Open(other_join_conjunct_evaluators_, state));
...
void PartitionedHashJoinNode::Close(RuntimeState* state) {
  ...
  ScalarExprEvaluator::Close(other_join_conjunct_evaluators_, state);
  ...
  while (!eos && !out_batch->AtCapacity()) {
    ...
    for (; !output_unmatched_batch_iter_->AtEnd() && !out_batch->AtCapacity();
      ...
      if (ExecNode::EvalConjuncts(
              conjunct_evaluators, num_conjuncts, out_batch_iterator.Get())) {
...
Status PartitionedHashJoinNode::OutputNullAwareProbeRows(RuntimeState* state,
  ...
  ScalarExprEvaluator* const* join_conjunct_evaluators =
