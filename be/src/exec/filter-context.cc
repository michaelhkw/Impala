...
Status FilterContext::CloneFrom(ObjectPool* pool, RuntimeState* state,
    MemPool* mem_pool, const FilterContext& from) {
  expr = from.expr;
  filter = from.filter;
  stats = from.stats;
  return from.expr_evaluator->Clone(pool, state, mem_pool, &expr_evaluator);
}

bool FilterContext::Eval(TupleRow* row) const noexcept {
  void* e = expr_evaluator->GetValue(row);
  DCHECK(&expr_evaluator->root() == expr);
  return filter->Eval(e, expr->type());
}
...
void FilterContext::Insert(TupleRow* row) const noexcept {
  ...
  uint32_t filter_hash = RawValue::GetHashValue(
      e, expr->type(), RuntimeFilterBank::DefaultHashSeed());
...
// entry:
//   %0 = alloca i16
//   %expr_evaluator_ptr = getelementptr inbounds %"struct.impala::FilterContext",
//       %"struct.impala::FilterContext"* %this, i32 0, i32 0
//   %expr_evaluator_arg = load %"class.impala::ExprContext"*,
//       %"class.impala::ExprContext"** %expr_evaluator_ptr
//   %result = call i32 @GetSlotRef(%"class.impala::ExprContext"* %expr_evaluator_arg,
...
Status FilterContext::CodegenEval(LlvmCodeGen* codegen, Function** fn) const {
  ...
  *fn = NULL;
  ...
  // Load 'expr_evaluator' from 'this_arg' FilterContext object.
  Value* expr_evaluator_ptr =
      builder.CreateStructGEP(nullptr, this_arg, 1, "expr_evaluator_ptr");
  Value* expr_evaluator_arg =
      builder.CreateLoad(expr_evaluator_ptr, "expr_evaluator_arg");

  // Evaluate the row against the filter's expression.
  Value* compute_fn_args[] = {expr_evaluator_arg, row_arg};
  ...
  Value* filter_ptr = builder.CreateStructGEP(NULL, this_arg, 2, "filter_ptr");
