...
bool FilterContext::Eval(TupleRow* row) const noexcept {
  void* val = expr_evaluator->GetValue(row);
  return filter->Eval(val, expr_evaluator->root().type());
}
...
void FilterContext::Insert(TupleRow* row) const noexcept {
  ...
  uint32_t filter_hash = RawValue::GetHashValue(
      val, expr_evaluator->root().type(), RuntimeFilterBank::DefaultHashSeed());
...
Status FilterContext::CodegenEval(LlvmCodeGen* codegen, ScalarExpr* filter_expr,
  ...
  *fn = nullptr;
  ...
  Value* expr_evaluator_ptr =
      builder.CreateStructGEP(nullptr, this_arg, 0, "expr_evaluator_ptr");
