...
Status SortNode::Prepare(RuntimeState* state) {
  ...
  sorter_.reset(new Sorter(*(less_than_.get()), slot_materialize_exprs_,
      &row_descriptor_, mem_tracker(), runtime_profile(), state));
  RETURN_IF_ERROR(sorter_->Init(pool_, state, expr_mem_pool()));
...
Status SortNode::Open(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(sorter_->Open(state));
