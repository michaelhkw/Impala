...
Status HashJoinNode::GetNext(RuntimeState* state, RowBatch* out_batch, bool* eos) {
  ...
  *eos = false;
  ...
  DCHECK_EQ(num_other_conjuncts, other_join_conjunct_evaluators_.size());
  ...
  while (!eos_) {
    ...
    while (!hash_tbl_iterator_.AtEnd()) {
      ...
      if (!EvalConjuncts(other_join_conjunct_evaluators_.data(),
      ...
      hash_tbl_iterator_.Next<true>();
      if (EvalConjuncts(conjunct_evaluators_.data(), num_conjuncts, out_row)) {
