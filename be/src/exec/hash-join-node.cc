...
Status HashJoinNode::Init(const TPlanNode& tnode, RuntimeState* state) {
  ...
  for (int i = 0; i < eq_join_conjuncts.size(); ++i) {
    ...
    Status status = ScalarExpr::Create(eq_join_conjuncts[i].left, child(0)->row_desc(),
        state, &probe_expr);
    probe_exprs_.push_back(probe_expr);
    RETURN_IF_ERROR(status);
    ScalarExpr* build_expr;
    status = ScalarExpr::Create(eq_join_conjuncts[i].right, child(1)->row_desc(),
        state, &build_expr);
...
Status HashJoinNode::Prepare(RuntimeState* state) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(other_join_conjuncts_, state, pool_,
      expr_mem_pool(), &other_join_conjunct_evaluators_));
