...
Status HdfsTableSink::Init(RuntimeState* state, const TDataSink& tsink,
    const vector<TExpr>& thrift_output_exprs) {
  RETURN_IF_ERROR(DataSink::Init(state, tsink, thrift_output_exprs));
  DCHECK(tsink.__isset.table_sink);
  RETURN_IF_ERROR(ScalarExpr::Create(state->obj_pool(), state, row_desc_,
      tsink.table_sink.hdfs_table_sink.partition_key_exprs, &partition_key_exprs_));
  return Status::OK();
}
...
Status HdfsTableSink::Prepare(RuntimeState* state, MemTracker* parent_mem_tracker) {
  ...
  RETURN_IF_ERROR(ScalarExprEvaluator::Create(state->obj_pool(), state,
      expr_mem_pool(), partition_key_exprs_, &partition_key_expr_evaluators_));
  ...
  for (size_t i = 0; i < partition_key_expr_evaluators_.size(); ++i) {
    // Remember non-constant partition key exprs for building hash table of Hdfs files.
    if (!partition_key_expr_evaluators_[i]->root()->is_constant()) {
      dynamic_partition_key_expr_evaluators_.push_back(partition_key_expr_evaluators_[i]);
    }
  }
  // Sanity check.
  DCHECK_LE(partition_key_expr_evaluators_.size(), table_desc_->num_cols())
      << DebugString();
  DCHECK_EQ(partition_key_expr_evaluators_.size(), table_desc_->num_clustering_cols())
      << DebugString();
  DCHECK_GE(output_expr_evaluators_.size(),
...
Status HdfsTableSink::Open(RuntimeState* state) {
  ...
  DCHECK_EQ(partition_key_exprs_.size(), partition_key_expr_evaluators_.size());
  RETURN_IF_ERROR(ScalarExprEvaluator::Open(partition_key_expr_evaluators_, state));
  ...
  for (const HdfsTableDescriptor::PartitionIdToDescriptorMap::value_type& id_to_desc:
    ...
    if (id_to_desc.first == g_ImpalaInternalService_constants.DEFAULT_PARTITION_ID) {
      ...
      DCHECK_EQ(partition->partition_key_value_evaluators().size(),
          partition_key_expr_evaluators_.size());
      vector<ScalarExprEvaluator*> dynamic_partition_key_value_evaluators;
      for (size_t i = 0; i < partition_key_expr_evaluators_.size(); ++i) {
        // Remember non-constant partition key exprs for building hash table of Hdfs files
        DCHECK(partition_key_expr_evaluators_[i]->root() == partition_key_exprs_[i]);
        if (!partition_key_exprs_[i]->is_constant()) {
          dynamic_partition_key_value_evaluators.push_back(
              partition->partition_key_value_evaluators()[i]);
        } else {
          ...
          void* table_partition_key_value =
              partition->partition_key_value_evaluators()[i]->GetValue(nullptr);
          void* target_partition_key_value =
              partition_key_expr_evaluators_[i]->GetValue(nullptr);
          ...
          if (table_partition_key_value == nullptr
              ...
              || !RawValue::Eq(table_partition_key_value, target_partition_key_value,
                     partition_key_expr_evaluators_[i]->root()->type())) {
      ...
      if (relevant_partition) {
        ...
        GetHashTblKey(nullptr, dynamic_partition_key_value_evaluators, &key);
...
Status HdfsTableSink::WriteClusteredRowBatch(RuntimeState* state, RowBatch* batch) {
  ...
  DCHECK(!dynamic_partition_key_expr_evaluators_.empty());
  ...
  if (current_clustered_partition_ == nullptr) {
    ...
    GetHashTblKey(current_row, dynamic_partition_key_expr_evaluators_,
  ...
  GetHashTblKey(batch->GetRow(batch->num_rows() - 1),
      dynamic_partition_key_expr_evaluators_, &last_row_key);
  ...
  for (int i = 0; i < batch->num_rows(); ++i) {
    ...
    string key;
    GetHashTblKey(current_row, dynamic_partition_key_expr_evaluators_, &key);
    ...
    GetHashTblKey(current_row, dynamic_partition_key_expr_evaluators_, &debug_row_key);
...
Status HdfsTableSink::InitOutputPartition(RuntimeState* state,
  ...
  for (int j = 0; j < partition_key_expr_evaluators_.size(); ++j) {
    partition_name_ss << table_desc_->col_descs()[j].name() << "=";
    void* value = partition_key_expr_evaluators_[j]->GetValue(row);
    ...
    if (value == nullptr) {
      ...
      string value_str;
      partition_key_expr_evaluators_[j]->PrintValue(value, &value_str);
...
void HdfsTableSink::GetHashTblKey(
    const TupleRow* row, const vector<ScalarExprEvaluator*>& ctxs, string* key) {
...
Status HdfsTableSink::Send(RuntimeState* state, RowBatch* batch) {
  SCOPED_TIMER(profile()->total_time_counter());
  ScalarExprEvaluator::FreeLocalAllocations(output_expr_evaluators_);
  ScalarExprEvaluator::FreeLocalAllocations(partition_key_expr_evaluators_);
  ...
  if (dynamic_partition_key_expr_evaluators_.empty()) {
...
void HdfsTableSink::Close(RuntimeState* state) {
  ...
  ScalarExprEvaluator::Close(partition_key_expr_evaluators_, state);
