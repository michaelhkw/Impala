...
class HdfsScanNodeBase : public ScanNode {
 public:
  ...
  const std::vector<ScalarExprEvaluator*>& min_max_conjunct_evaluators() const {
    return min_max_conjunct_evaluators_;
  }
  ...
  const ConjunctEvaluatorsMap& conjuncts_map() const { return conjunct_evaluators_map_; }
  ...
  Tuple* InitTemplateTuple(const std::vector<ScalarExprEvaluator*>& value_evaluators,
 ...
 protected:
  ...
  vector<ScalarExprEvaluator*> min_max_conjunct_evaluators_;
  ...
  ConjunctEvaluatorsMap conjunct_evaluators_map_;
  ...
  /// the per-scanner ScannerContext. Correspond with exprs in 'filter_exprs_'.
