...
Status UnionNode::Prepare(RuntimeState* state) {
  ...
  for (const vector<ScalarExpr*>& const_exprs : const_exprs_lists_) {
    ...
    RETURN_IF_ERROR(ScalarExprEvaluator::Create(const_exprs, state, pool_,
        expr_mem_pool(), &const_expr_evaluators));
    AddEvaluatorsToFree(const_expr_evaluators);
    const_expr_evaluators_lists_.push_back(const_expr_evaluators);
  }
  ...
  for (const vector<ScalarExpr*>& child_exprs : child_exprs_lists_) {
    ...
    RETURN_IF_ERROR(ScalarExprEvaluator::Create(child_exprs, state, pool_,
        expr_mem_pool(), &child_expr_evaluators));
    AddEvaluatorsToFree(child_expr_evaluators);
    child_expr_evaluators_lists_.push_back(child_expr_evaluators);
  }
  return Status::OK();
}
...
Status UnionNode::Open(RuntimeState* state) {
  ...
  for (vector<ScalarExprEvaluator*>& evaluators : const_expr_evaluators_lists_) {
    RETURN_IF_ERROR(ScalarExprEvaluator::Open(evaluators, state));
  }
  // Open result expr lists.
  for (vector<ScalarExprEvaluator*>& evaluators : child_expr_evaluators_lists_) {
    RETURN_IF_ERROR(ScalarExprEvaluator::Open(evaluators, state));
  }
...
Status UnionNode::GetNextConst(RuntimeState* state, RowBatch* row_batch) {
  ...
  DCHECK_LT(const_exprs_lists_idx_, const_expr_evaluators_lists_.size());
  ...
  while (const_exprs_lists_idx_ < const_exprs_lists_.size() && !row_batch->AtCapacity()) {
    MaterializeExprs(
        const_expr_evaluators_lists_[const_exprs_lists_idx_], nullptr, tuple_buf, row_batch);
...
void UnionNode::Close(RuntimeState* state) {
  ...
  for (vector<ScalarExpr*>& const_exprs : const_exprs_lists_) {
    ScalarExpr::Close(const_exprs);
  }
  for (vector<ScalarExpr*>& child_exprs : child_exprs_lists_) {
