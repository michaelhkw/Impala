...
Status HdfsScanNode::Prepare(RuntimeState* state) {
  ...
  row_batches_get_timer_ =
      ADD_COUNTER(runtime_profile(), "RowBatchQueueGetWaitTime", TUnit::TIME_NS);
  row_batches_put_timer_ =
      ADD_COUNTER(runtime_profile(), "RowBatchQueuePutWaitTime", TUnit::TIME_NS);
  return Status::OK();
}
