...
HdfsScanner::HdfsScanner(HdfsScanNodeBase* scan_node, RuntimeState* state)
    : scan_node_(scan_node),
      ...
      stream_(NULL),
      eos_(false),
      expr_mem_pool_(new MemPool(scan_node->mem_tracker())),
      conjunct_evaluators_(NULL),
...
Status HdfsScanner::Open(ScannerContext* context) {
  ...
  for (const auto& entry: scan_node_->conjuncts_map()) {
    RETURN_IF_ERROR(ScalarExprEvaluator::Clone(&obj_pool_, scan_node_->runtime_state(),
        ...
        &conjunct_evaluators_map_[entry.first]));
  }
  DCHECK(conjunct_evaluators_map_.find(scan_node_->tuple_desc()->id()) !=
         conjunct_evaluators_map_.end());
  conjunct_evaluators_ = &conjunct_evaluators_map_[scan_node_->tuple_desc()->id()];

  // Setup the scan node's dictionary filtering conjuncts map.
  if (scan_node_->thrift_dict_filter_conjuncts_map() != nullptr) {
    for (auto& entry : *(scan_node_->thrift_dict_filter_conjuncts_map())) {
      ...
      // into conjunct_evaluators_.
      for (int conjunct_idx : entry.second) {
        DCHECK_LT(conjunct_idx, conjunct_evaluators_->size());
        DCHECK((*conjunct_evaluators_)[conjunct_idx] != nullptr);
        dict_filter_map_[entry.first].push_back((*conjunct_evaluators_)[conjunct_idx]);
      }
    }
  }
  ...
  template_tuple_ = scan_node_->InitTemplateTuple(
      context_->partition_descriptor()->partition_key_value_evaluators(),
...
Status HdfsScanner::CommitRows(int num_rows, bool enqueue_if_full, RowBatch* row_batch) {
  ...
  for (const auto& entry: conjunct_evaluators_map_) {
