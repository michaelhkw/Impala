...
HdfsSequenceTableWriter::HdfsSequenceTableWriter(HdfsTableSink* parent,
  ...
  : HdfsTableWriter(parent, state, output, partition, table_desc, output_expr_evaluators),
...
void HdfsSequenceTableWriter::EncodeRow(TupleRow* row, WriteStream* buf) {
  ...
  for (int j = 0; j < num_non_partition_cols; ++j) {
    void* value = output_expr_evaluators_[j]->GetValue(row);
    if (value != NULL) {
      if (output_expr_evaluators_[j]->root().type().type == TYPE_STRING) {
        WriteEscapedString(reinterpret_cast<const StringValue*>(value), &row_buf_);
      } else {
        string str;
        output_expr_evaluators_[j]->PrintValue(value, &str);
