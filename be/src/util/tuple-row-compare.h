...
class TupleRowComparator {
 public:
  ...
  /// 'state' is passed in for initialization of the evaluator. Idempotent as Open()
  /// may be called multiple times in a subplan.
  ...
  int ALWAYS_INLINE Compare(const TupleRow* lhs, const TupleRow* rhs) const {
    return codegend_compare_fn_ == NULL ?
        ...
        (*codegend_compare_fn_)(&ordering_expr_evaluators_lhs_[0],
            &ordering_expr_evaluators_rhs_[0], lhs, rhs);
  }
  ...
  void FreeLocalAllocations() const {
    ScalarExprEvaluator::FreeLocalAllocations(ordering_expr_evaluators_lhs_);
    ScalarExprEvaluator::FreeLocalAllocations(ordering_expr_evaluators_rhs_);
  }
 ...
 private:
  ...
  std::vector<ScalarExprEvaluator*> ordering_expr_evaluators_lhs_;
  std::vector<ScalarExprEvaluator*> ordering_expr_evaluators_rhs_;
