...
Status TupleRowComparator::Open(ObjectPool* pool, RuntimeState* state, MemPool* mem_pool) {
  if (!opened_) {
    RETURN_IF_ERROR(ScalarExprEvaluator::Create(pool, state, mem_pool,
        ordering_exprs_, &key_expr_evaluators_lhs_));
    RETURN_IF_ERROR(ScalarExprEvaluator::Open(key_expr_evaluators_lhs_, state));
    RETURN_IF_ERROR(ScalarExprEvaluator::Clone(pool, state, mem_pool,
        key_expr_evaluators_lhs_, &key_expr_evaluators_rhs_));
    opened_ = true;
  }
  DCHECK_EQ(ordering_exprs_.size(), key_expr_evaluators_lhs_.size());
  DCHECK_EQ(key_expr_evaluators_lhs_.size(), key_expr_evaluators_rhs_.size());
  return Status::OK();
}

void TupleRowComparator::Close(RuntimeState* state) {
  ScalarExprEvaluator::Close(key_expr_evaluators_rhs_, state);
  ScalarExprEvaluator::Close(key_expr_evaluators_lhs_, state);
}
...
int TupleRowComparator::CompareInterpreted(
  ...
  DCHECK_EQ(ordering_exprs_.size(), key_expr_evaluators_lhs_.size());
  DCHECK_EQ(key_expr_evaluators_lhs_.size(), key_expr_evaluators_rhs_.size());
  for (int i = 0; i < key_expr_evaluators_lhs_.size(); ++i) {
    void* lhs_value = key_expr_evaluators_lhs_[i]->GetValue(lhs);
    void* rhs_value = key_expr_evaluators_rhs_[i]->GetValue(rhs);
...
// define i32 @Compare(%"class.impala::ScalarExprEvaluator"** %key_expr_evaluators_lhs,
//                     %"class.impala::ScalarExprEvaluator"** %key_expr_evaluators_rhs,
...
//   %2 = alloca i32
//   %3 = alloca i32
//   %4 = getelementptr %"class.impala::ScalarExprEvaluator"** %key_expr_evaluators_lhs, i32 0
...
//   %6 = getelementptr %"class.impala::ScalarExprEvaluator"** %key_expr_evaluators_rhs, i32 0
...
//   %15 = getelementptr %"class.impala::ScalarExprEvaluator"** %key_expr_evaluators_lhs, i32 1
...
//   %17 = getelementptr %"class.impala::ScalarExprEvaluator"** %key_expr_evaluators_rhs, i32 1
...
Status TupleRowComparator::CodegenCompare(LlvmCodeGen* codegen, Function** fn) {
  ...
  // int Compare(ScalarExprEvaluator** key_expr_evaluators_lhs,
  //     ScalarExprEvaluator** key_expr_evaluators_rhs,
  //     TupleRow* lhs, TupleRow* rhs)
  PointerType* expr_evaluators_type =
  ...
  prototype.AddArgument("key_expr_evaluators_lhs", expr_evaluators_type);
  prototype.AddArgument("key_expr_evaluators_rhs", expr_evaluators_type);
  ...
  *fn = prototype.GeneratePrototype(&builder, args);
  Value* lhs_evaluators_arg = args[0];
  Value* rhs_evaluators_arg = args[1];
  ...
  for (int i = 0; i < ordering_exprs.size(); ++i) {
    ...
    // Call key_fns[i](key_expr_evaluators_lhs[i], lhs_arg)
    Value* lhs_evaluator = codegen->CodegenArrayAt(&builder, lhs_evaluators_arg, i);
    Value* lhs_args[] = { lhs_evaluator, lhs_arg };
    ...
    // Call key_fns[i](key_expr_evaluators_rhs[i], rhs_arg)
    Value* rhs_evaluator = codegen->CodegenArrayAt(&builder, rhs_evaluators_arg, i);
    Value* rhs_args[] = { rhs_evaluator, rhs_arg };
