...
Status TupleRowComparator::Open(
    ObjectPool* pool, RuntimeState* state, MemPool* mem_pool) {
  if (!opened_) {
    RETURN_IF_ERROR(ScalarExprEvaluator::Create(ordering_exprs_, state, pool, mem_pool,
        &ordering_expr_evaluators_lhs_));
    RETURN_IF_ERROR(ScalarExprEvaluator::Open(ordering_expr_evaluators_lhs_, state));
    RETURN_IF_ERROR(ScalarExprEvaluator::Clone(pool, state, mem_pool,
        ordering_expr_evaluators_lhs_, &ordering_expr_evaluators_rhs_));
    opened_ = true;
  }
  DCHECK_EQ(ordering_exprs_.size(), ordering_expr_evaluators_lhs_.size());
  DCHECK_EQ(ordering_expr_evaluators_lhs_.size(), ordering_expr_evaluators_rhs_.size());
  return Status::OK();
}

void TupleRowComparator::Close(RuntimeState* state) {
  ScalarExprEvaluator::Close(ordering_expr_evaluators_rhs_, state);
  ScalarExprEvaluator::Close(ordering_expr_evaluators_lhs_, state);
  opened_ = false;
}
...
int TupleRowComparator::CompareInterpreted(
  ...
  DCHECK_EQ(ordering_exprs_.size(), ordering_expr_evaluators_lhs_.size());
  DCHECK_EQ(ordering_expr_evaluators_lhs_.size(), ordering_expr_evaluators_rhs_.size());
  for (int i = 0; i < ordering_expr_evaluators_lhs_.size(); ++i) {
    void* lhs_value = ordering_expr_evaluators_lhs_[i]->GetValue(lhs);
    void* rhs_value = ordering_expr_evaluators_rhs_[i]->GetValue(rhs);
...
//                         %ordering_expr_evaluators_lhs,
//                     %"class.impala::ScalarExprEvaluator"**
//                         %ordering_expr_evaluators_rhs,
...
//            %ordering_expr_evaluators_lhs, i32 0
...
//            %ordering_expr_evaluators_rhs, i32 0
...
//             %ordering_expr_evaluators_lhs, i32 1
...
//            %ordering_expr_evaluators_rhs, i32 1
...
Status TupleRowComparator::CodegenCompare(LlvmCodeGen* codegen, Function** fn) {
  ...
  // int Compare(ScalarExprEvaluator** ordering_expr_evaluators_lhs,
  //     ScalarExprEvaluator** ordering_expr_evaluators_rhs,
  ...
  prototype.AddArgument("ordering_expr_evaluators_lhs", expr_evaluators_type);
  prototype.AddArgument("ordering_expr_evaluators_rhs", expr_evaluators_type);
  ...
  *fn = prototype.GeneratePrototype(&builder, args);
  ...
  for (int i = 0; i < ordering_exprs.size(); ++i) {
    ...
    // Call key_fns[i](ordering_expr_evaluators_lhs[i], lhs_arg)
    ...
    // Call key_fns[i](ordering_expr_evaluators_rhs[i], rhs_arg)
