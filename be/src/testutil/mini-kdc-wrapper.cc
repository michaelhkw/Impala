...
Status MiniKdcWrapper::StartKdc(const string& keytab_dir) {
...
Status MiniKdcWrapper::SetupAndStartMiniKDC(const string& realm,
    const string& ticket_lifetime, const string& renew_lifetime,
    int kdc_port, unique_ptr<MiniKdcWrapper>* kdc_ptr) {
  unique_ptr<MiniKdcWrapper> kdc(new MiniKdcWrapper(
      realm, ticket_lifetime, renew_lifetime, kdc_port));
