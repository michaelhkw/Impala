...
/// the expressions in 'slot_materialize_exprs_' or "intermediate runs" constructed
...
class Sorter::Run {
 public:
  ...
  /// sorter_->slot_materialize_expr_evaluators_, while AddIntermediateBatch() just copies rows.
    ...
    for (int i = 0; i < tuples_remaining; ++i) {
      ...
      if (INITIAL_RUN) {
        new_tuple->MaterializeExprs<HAS_VAR_LEN_SLOTS, true>(input_row,
            *sort_tuple_desc_, sorter_->slot_materialize_expr_evaluators_, NULL,
...
Sorter::Sorter(TupleRowComparator& compare_less_than,
    const vector<ScalarExpr*>& slot_materialize_exprs,
  ...
  : state_(state),
    ...
    slot_materialize_exprs_(slot_materialize_exprs),
...
void Sorter::FreeLocalAllocations() {
  ...
  ScalarExprEvaluator::FreeLocalAllocations(slot_materialize_expr_evaluators_);
}
