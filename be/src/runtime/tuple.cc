...
void Tuple::MaterializeExprs(TupleRow* row, const TupleDescriptor& desc,
    ScalarExprEvaluator* const* evaluators, MemPool* pool,
    StringValue** non_null_string_values, int* total_string_lengths,
    int* num_non_null_string_values) {
  ClearNullBits(desc);
  // Evaluate the materialize_expr_evaluators and place the results in the tuple.
  for (int i = 0; i < desc.slots().size(); ++i) {
    ...
    DCHECK(slot_desc->type().type == TYPE_NULL ||
        slot_desc->type() == evaluators[i]->root()->type());
    void* src = evaluators[i]->GetValue(row);
    if (src != NULL) {
...
//     %"class.impala::ScalarExprEvaluator"** %materialize_expr_evaluators,
...
//   %1 = getelementptr %"class.impala::ScalarExprEvaluator"** %materialize_expr_evaluators, i32 0
//   %expr_evaluator = load %"class.impala::ScalarExprEvaluator"** %1
//   %src = call i64 @GetSlotRef4(%"class.impala::ScalarExprEvaluator"* %expr_evaluator,
...
//   %4 = getelementptr %"class.impala::ScalarExprEvaluator"** %materialize_expr_evaluators, i32 1
//   %expr_evaluator1 = load %"class.impala::ScalarExprEvaluator"** %4
//   %src2 = call { i64, i8* } @GetSlotRef5(%"class.impala::ScalarExprEvaluator"* %expr_evaluator1,
//       %"class.impala::TupleRow"* %row)
//   ; ----- generated by CodegenAnyVal::WriteToSlot() ----------------------------------
...
Status Tuple::CodegenMaterializeExprs(LlvmCodeGen* codegen, bool collect_string_vals,
  ...
  prototype.AddArgument("slot_materialize_exprs", expr_evaluators_type);
  ...
  *fn = prototype.GeneratePrototype(&builder, args);
  ...
  Value* expr_evaluators_arg = args[3];
  ...
  for (int i = 0; i < desc.slots().size(); ++i) {
    ...
    Value* expr_evaluator =
        codegen->CodegenArrayAt(&builder, expr_evaluators_arg, i, "expr_evaluator");
    Value* expr_args[] = { expr_evaluator, row_arg };
