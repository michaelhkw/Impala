...
class RuntimeState {
 public:
  ...
  /// Returns true if there are ScalarFnCall expressions in the fragments which
  /// can't be interpreted.
  bool HasScalarFnToCodegen() const { return !scalar_fns_to_codegen_.empty(); }

  /// Returns true if codegen is disabled due to hints. This can happen for single node
  ...
  /// 1. it's not disabled by query option
  ...
  inline bool ShouldCodegen() const {
    return !CodegenDisabledByQueryOption() &&
        (!CodegenDisabledByHint() || HasScalarFnToCodegen());
  }
