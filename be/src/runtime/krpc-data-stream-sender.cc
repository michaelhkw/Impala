...
DECLARE_int32(backend_client_connection_num_retries);
...
//
// Each channel internally has two protobuf row batches to serialize to. This allows
// client to serialize the next row batch while the current row batch is being sent.
// Upon completion of a RPC, the callback TransmitDataCompleteCb() will be invoked.
...
//
// When the data stream sender is shut down, it will call TearDown() on all channels to
...
// thread to call FlushAndSendEos() before closing the data stream sender to send all
...
// will relinquish ownership of them before the completion callback is invoked so it's
// safe to free them.
class KrpcDataStreamSender::Channel : public CacheLineAligned {
 ...
 private:
  ...
  // Returns the index of next entry to use in 'cached_proto_batches_' for
  // TransmitData() RPC.
  ...
  void RetryCb(const kudu::Status& status);

  // A callback function called from KRPC reactor thread upon completion of the in-flight
  // RPC. This may be called when the remote server responds to the RPC or when the RPC
  // ends prematurely due to various reasons (e.g. cancellation, time-out). If it turns
  // out that the RPC failed because the remote server is too busy, this function will
  // reschedule a callback to retry the RPC. Otherwise, it will call MarkDone() to mark
  // the in-flight RPC as complete.
  void TransmitDataCompleteCb();

  // Initializes the parameters for TransmitData() RPC and invokes the RPC call.
  ...
  // completion. Expects to be called with 'lock_' held.
...
void KrpcDataStreamSender::Channel::MarkDone(const Status& status) {
  ...
  // Clear the header to indicate the proto-batch is not in use.
  DCHECK(proto_batch->IsInitialized());
  proto_batch->header.Clear();
  rpc_status_ = status;
  rpc_in_flight_ = false;
  rpc_done_cv_.notify_one();
}

void KrpcDataStreamSender::Channel::RetryCb(const kudu::Status& cb_status) {
  ...
  // Retry the RPC.
  Status status = DoTransmitDataRpc();
  ...
  if (LIKELY(controller_status.ok())) {
    ...
    if (RpcMgr::IsServerTooBusy(rpc_controller_) && !ShouldTerminate()) {
      ...
      rpc_mgr->messenger()->ScheduleOnReactor(
          boost::bind(&KrpcDataStreamSender::Channel::RetryCb, this, _1),
          MonoDelta::FromMilliseconds(FLAGS_rpc_retry_interval_ms));
      return;
    }
    rpc_status = FromKuduStatus(controller_status);
  }
  MarkDone(rpc_status);
}
...
Status KrpcDataStreamSender::Channel::DoTransmitDataRpc() {
  ProtoRowBatch* proto_batch = CurrentProtoBatch();
  DCHECK(proto_batch->header.has_num_rows());
  DCHECK(proto_batch->header.has_uncompressed_size());
  DCHECK(proto_batch->header.has_compression_type());
...
Status KrpcDataStreamSender::Channel::WaitForRpc(std::unique_lock<SpinLock>* lock) {
  ...
  if (UNLIKELY(ShouldTerminate())) {
    // DSS is single-threaded so it's impossible for shutdown_ to betrue here.
    DCHECK(!shutdown_);
...
Status KrpcDataStreamSender::Channel::FlushAndSendEos(RuntimeState* state) {
  ...
  MonoDelta timeout = MonoDelta::FromMilliseconds(FLAGS_backend_client_rpc_timeout_ms);
  for (int i = 0; i < FLAGS_backend_client_connection_num_retries; ++i) {
    rpc_controller_.Reset();
    rpc_controller_.set_timeout(timeout);
    EndDataStreamResponsePB eos_resp;
    proxy->EndDataStream(eos_req, &eos_resp, &rpc_controller_);

    // RPC suceeded. Done here.
    if (rpc_controller_.status().ok()) break;
    ...
    // RPC failed.
    rpc_status_ = FromKuduStatus(rpc_controller_.status());
...
void KrpcDataStreamSender::Channel::Teardown(RuntimeState* state) {
  ...
  // all the data should already be drained.
  std::unique_lock<SpinLock> l(lock_);
  shutdown_ = true;
...
Status KrpcDataStreamSender::Prepare(RuntimeState* state, MemTracker* parent_mem_tracker) {
