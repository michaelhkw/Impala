...
class KrpcDataStreamSender::Channel : public CacheLineAligned {
 public:
  ...
  Channel(KrpcDataStreamSender* parent, const RowDescriptor* row_desc,
    ...
    : parent_(parent),
      ...
      rpc_mgr_(ExecEnv::GetInstance()->rpc_mgr()) {
    DCHECK(rpc_mgr_ != nullptr);
 ...
 private:
  ...
  // XXX
  CachedProtoRowBatch cached_proto_batches_[2];
  ...
  SpinLock lock_; // Lock with rpc_done_cv_ protecting rpc_in_flight_

  condition_variable_any rpc_done_cv_;   // signaled when rpc_in_flight_ is set to true.
  bool rpc_in_flight_ = false;  // true if the rpc_thread_ is busy sending.
  ...
  // Returns true if the channel should terminate because parent's sender has been
  // closed or cancelled.
  ...
  Status SendBatch(CachedProtoRowBatch* batch) WARN_UNUSED_RESULT;

  // Waits for the current RPC to complete.
...
Status KrpcDataStreamSender::Channel::Init(RuntimeState* state) {
  ...
  /* XXX */
  RETURN_IF_ERROR(rpc_mgr_->GetProxy(address_, &proxy));
...
bool KrpcDataStreamSender::Channel::IsRemoteBusy() {
  const kudu::Status status = rpc_controller_.status();
  const kudu::rpc::ErrorStatusPB* err = rpc_controller_.error_response();
...
void KrpcDataStreamSender::Channel::TransmitDataCompleteCb() {
  ...
  if (LIKELY(controller_status.ok())) {
    ...
    if (IsRemoteBusy() && !ShouldTerminate()) {
      int num_ms = 5 + (rand() % 5);
      rpc_mgr_->messenger()->ScheduleOnReactor(
...
Status KrpcDataStreamSender::Channel::DoTransmitDataRpc() {
  ...
  // Add the tuple_offsets_ as sidecar.
  int sidecar_idx;
...
Status KrpcDataStreamSender::Channel::TransmitData() {
  ...
  DCHECK(!rpc_in_flight_);
  DCHECK(!CurrentProtoBatch()->header.has_num_rows());
  rpc_in_flight_ = true;
