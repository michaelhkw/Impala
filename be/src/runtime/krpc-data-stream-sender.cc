...
class KrpcDataStreamSender::Channel : public CacheLineAligned {
 ...
 private:
  ...
  // XXX
  std::deque<OutboundRowBatch*> pending_rpc_queue_;
...
void KrpcDataStreamSender::Channel::MarkDone(const Status& status) {
  ...
  if (!pending_rpc_queue_.empty()) {
    pending_rpc_queue_.front()->DecUse();
    pending_rpc_queue_.pop_front();
  }
...
Status KrpcDataStreamSender::Channel::DoTransmitDataRpc() {
  ...
  if (UNLIKELY(remote_recvr_closed_)) {
    MarkDone(Status::OK());
    return Status::OK();
  }

  OutboundRowBatch* current_batch = pending_rpc_queue_.front();
  DCHECK(current_batch->IsInitialized());
  DCHECK(current_batch->IsInUse());
...
Status KrpcDataStreamSender::Channel::TransmitData(
  ...
  DCHECK(!IsPendingQueueFull());
  pending_rpc_queue_.push_back(const_cast<OutboundRowBatch*>(outbound_batch));
...
Status KrpcDataStreamSender::Channel::SerializeAndSendBatch(RowBatch* batch) {
  ...
  DCHECK(!outbound_batch->IsInUse());
  outbound_batch->IncUse();
...
Status KrpcDataStreamSender::Send(RuntimeState* state, RowBatch* batch) {
  ...
  if (partition_type_ == TPartitionType::UNPARTITIONED) {
    OutboundRowBatch* outbound_batch = &outbound_batches_[next_batch_idx_];
    DCHECK(!outbound_batch->IsInUse());
    outbound_batch->IncUse(channels_.size());
