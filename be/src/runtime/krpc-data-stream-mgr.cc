...
shared_ptr<DataStreamRecvrBase> KrpcDataStreamMgr::CreateRecvr(
  ...
  for (const unique_ptr<EndDataStreamCtx>& ctx :
    ...
    DataStreamService::RespondDeferredRpc(Status::OK(), ctx->response, ctx->rpc_context,
        mem_tracker_.get());
    num_senders_waiting_->Increment(-1);
  }
  return recvr;
...
void KrpcDataStreamMgr::AddData(const TransmitDataRequestPB* request,
  ...
  if (already_unregistered) {
    ...
    DataStreamService::RespondDeferredRpc(Status::Expected(msg), response, rpc_context,
...
void KrpcDataStreamMgr::CloseSender(const EndDataStreamRequestPB* request,
  ...
  DataStreamService::RespondDeferredRpc(Status::OK(), response, rpc_context,
      service_mem_tracker_);
}
...
void KrpcDataStreamMgr::RespondToTimedOutSender(const std::unique_ptr<ContextType>& ctx) {
  ...
  VLOG_QUERY << msg.msg();
  DataStreamService::RespondDeferredRpc(Status::Expected(msg), ctx->response,
