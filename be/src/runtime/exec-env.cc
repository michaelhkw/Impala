...
Status ExecEnv::StartServices() {
  ...
  IpAddr ip;
  RETURN_IF_ERROR(HostnameToIpAddr(backend_address_.hostname, &ip));
  if (FLAGS_use_krpc) {
    krpc_address_.__set_hostname(ip);
  ...
  if (scheduler_ != nullptr) {
    RETURN_IF_ERROR(scheduler_->Init(backend_address_, krpc_address_, ip));
  }
