...
Status KrpcDataStreamRecvr::SenderQueue::AddBatchWork(int64_t batch_size,
  ...
  Status status;
  {
    ...
    status = RowBatch::Create(recvr_->row_desc(), header, tuple_offsets, tuple_data,
        recvr_->mem_tracker(), recvr_->client(), recvr_->tuple_data_reservation(), &batch);
  }
  lock->lock();
...
KrpcDataStreamRecvr::KrpcDataStreamRecvr(KrpcDataStreamMgr* stream_mgr,
  ...
  : mgr_(stream_mgr),
    ...
    client_(recvr_buffer_pool_client),
    profile_(profile),
