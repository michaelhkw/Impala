...
class FragmentInstanceState {
 public:
  ...
  /// Called periodically to get the current status of this fragment instance.
  /// The fragment instance's status is stored in 'instance_status' and its Thrift
  /// runtime profile is stored in 'thrift_profile.
 ...
 private:
  ...
  /// Monontonically increasing sequence number used in status report to prevent
  /// duplicated or out-of-order reports.
  int32_t report_seq_no_ = 0;

  /// True iff the final report has already been sent.
  ...
  /// Returns the monotonically increasing sequence number. Called by query state thread.
  int32_t AdvanceReportSeqNo() { return ++report_seq_no_; }
