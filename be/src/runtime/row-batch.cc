...
Status RowBatch::Deserialize(const kudu::Slice& input_tuple_offsets,
  ...
  if (client != nullptr) {
    ...
    BufferPool::BufferHandle* tuple_ptrs_buffer = &(tuple_ptrs_info_->buffer);
    RETURN_IF_ERROR(AllocateBuffer(client, tuple_ptrs_size_, tuple_ptrs_buffer));
    tuple_ptrs_ = reinterpret_cast<Tuple**>(tuple_ptrs_buffer->data());

    BufferPool::BufferHandle tuple_data_buffer;
    RETURN_IF_ERROR(AllocateBuffer(client, uncompressed_size, &tuple_data_buffer));
    tuple_data = tuple_data_buffer.data();
    AddBuffer(client, move(tuple_data_buffer), FlushMode::NO_FLUSH_RESOURCES);
  } else {
    mem_tracker_->Consume(tuple_ptrs_size_);
    tuple_ptrs_ = reinterpret_cast<Tuple**>(malloc(tuple_ptrs_size_));
...
Status RowBatch::Create(const RowDescriptor* row_desc, const RowBatchHeaderPB& header,
    ...
    std::unique_ptr<RowBatch>* row_batch_ptr) {
...
Status RowBatch::AllocateBuffer(BufferPool::ClientHandle* client, int64_t len,
  ...
  if (UNLIKELY(!buffer_handle->is_open())) {
    ErrorMsg msg(TErrorCode::ROW_BATCH_ALLOC_FAIL, buffer_len);
    VLOG_QUERY << msg.msg();
...
void RowBatch::AcquireState(RowBatch* src) {
  ...
  std::swap(tuple_ptrs_info_, src->tuple_ptrs_info_);
