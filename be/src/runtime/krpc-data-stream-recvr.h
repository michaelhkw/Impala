...
/// KrpcDataStreamMgr from one or more sender fragment instances.
/// Receivers are created via KrpcDataStreamMgr::CreateRecvr().
/// Ownership of a stream recvr is shared between the KrpcDataStreamMgr that created it and
/// the caller of KrpcDataStreamMgr::CreateRecvr() (i.e. the exchange node)
//
...
//
/// KrpcDataStreamRecvr::Close() must be called by the caller of CreateRecvr() to remove the
/// recvr instance from the tracking structure of its KrpcDataStreamMgr in all cases.
