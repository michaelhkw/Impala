...
class QueryState {
 public:
  ...
  /// state accordingly. This is also responsible for sending status report periodically
 ...
 private:
  ...
  /// 'overall_status_' will be set once this is unblocked and so will 'failed_instance_id_'
  /// if an error is hit.
  ...
  /// Returns true if the overall backend status is already set.
  ...
  /// Updates the BackendExecState based on 'overall_status_'. A state transition happens
  /// if the current state is a non-terminal state; the transition can either be to the
  /// next legal state or ERROR if 'overall_status_' is an error. Called by the query
  /// state thread only. It acquires the 'status_lock_' to synchronize with the fragment
  /// instance threads' setting the status.
  ///
