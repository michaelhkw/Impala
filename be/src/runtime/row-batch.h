...
class RowBatch {
 public:
  ...
  /// Creates a row batch from the serialized row batch header, decompress / copy
  ...
  /// buffer pool with 'buffer_pool_client' as client. The newly created row batch is
  ...
  static Status Create(const RowDescriptor* row_desc, const RowBatchHeaderPB& header,
      ...
      std::unique_ptr<RowBatch>* row_batch_ptr) WARN_UNUSED_RESULT;
 ...
 private:
  ...
  /// into pointers into the tuple data's buffer. Called from Create() above.
  RowBatch(const RowDescriptor* row_desc, const RowBatchHeaderPB& header,
      const kudu::Slice& input_tuple_data, const kudu::Slice& input_tuple_offsets,
      MemTracker* mem_tracker);

  /// Allocate from buffer pool a buffe of 'len' using the client handle 'client'.
  /// Note that 'len' is rounded up to the next power of 2 or minimum buffer size,
  /// whichever is larger.
  ...
  Status Deserialize(const kudu::Slice& input_tuple_offsets,
      ...
      BufferPool::ClientHandle* client = nullptr) WARN_UNUSED_RESULT;
